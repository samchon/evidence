...
import { EvidConfigLoader } from "./loaders/EvidConfigLoader";
import { EvidCheckProgrammer } from "./programmers/EvidCheckProgrammer";
...
export class EvidChecker {
  ...
  public async evaluate(input: IEvidConfigPlan): Promise<IEvidCheckAnalysis> {
    ...
    const context: IEvidCheckContext = {
      plan,
      claims: await EvidCheckProgrammer.materialize(plan),
    };
    return EvidCheckProgrammer.evaluate(context);
  }
  ...
  public static async evaluate(
    ...
    return new EvidChecker(input.configFile).evaluate(input);
  }
}
