...
import { EvidEcmaScriptExportResolver } from "./EvidEcmaScriptExportResolver";
import { EvidEcmaScriptFileScanner } from "./EvidEcmaScriptFileScanner";
import { EvidEcmaScriptModuleResolver } from "./EvidEcmaScriptModuleResolver";
...
export class EvidEcmaScriptAdapter implements IEvidAdapter {
  ...
  public async analyze(
    ...
    const moduleResolution =
      this.type === "javascript"
        ? await new EvidEcmaScriptModuleResolver().resolve(input.files)
        : undefined;
    const sourceDiagnostics: IEvidInventory["diagnostics"] =
    ...
    try {
      ...
      const published = new EvidEcmaScriptExportResolver(
        analyses,
        inventory,
        input.root,
        this.type,
  ...
  private async scan(
    ...
    mode: IEvidEcmaScriptFileAnalysis["mode"],
  ): Promise<IEvidEcmaScriptFileAnalysis> {
  ...
  private host(
    source: IEvidSourceFile,
    comment: IEvidEcmaScriptComment,
