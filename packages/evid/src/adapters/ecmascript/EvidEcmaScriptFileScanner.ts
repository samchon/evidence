...
import type { IEvidEcmaScriptStatementContext } from "./IEvidEcmaScriptStatementContext";
...
export class EvidEcmaScriptFileScanner {
   ...
   */
  private readonly units = new Map<string, IEvidEcmaScriptOwnedUnit>();
  private readonly excludedRoots = new Set<string>();
  private readonly comments = new Map<string, IEvidEcmaScriptComment>();
  private readonly exports: IEvidEcmaScriptExport[] = [];
  private readonly positions = new Map<string, IEvidEcmaScriptHostPosition>();
  private readonly imports: IEvidEcmaScriptImport[] = [];
  private readonly diagnostics: IEvidDiagnostic[] = [];
  private readonly commonJsExports = new Map<string, IEvidEcmaScriptExport>();
  ...
  public constructor(
    ...
    private readonly mode: EvidEcmaScriptModuleMode,
  ) {
    for (const node of session.root.descendantsOfType("comment")) {
      ...
      this.comments.set(this.commentKey(node), {
        ...
        range,
        syntax: EvidEcmaScriptSyntax.comment(node),
        attachments: [],
      });
    }
  }
  ...
  private collectImports(): void {
    for (const statement of this.session.root.namedChildren) {
      ...
      const specifier = EvidEcmaScriptSyntax.module(
      ...
      const statementTypeOnly = EvidEcmaScriptSyntax.token(statement, "type");
      ...
      for (const entry of named?.namedChildren ?? []) {
        ...
        const imported = EvidEcmaScriptSyntax.specifierName(entry);
        const local = EvidEcmaScriptSyntax.specifierAlias(entry) ?? imported;
  ...
  private scanStatements(
    block: EvidNode,
    inherited: IEvidEcmaScriptStatementContext,
  ): void {
    ...
    for (const statement of declarations) {
      ...
      const context: IEvidEcmaScriptStatementContext = {
        ...inherited,
        ambient:
        ...
        typeOnly:
          inherited.typeOnly ||
          (statement.type === "export_statement" &&
            EvidEcmaScriptSyntax.token(statement, "type")),
      };
  ...
  private scanInterface(
    ...
    const defaulted =
      ...
      EvidEcmaScriptSyntax.token(wrapper, "default");
  ...
  private scanClass(
    ...
    const declared = EvidEcmaScriptSyntax.name(
      declaration.childForFieldName("name"),
    );
    const defaulted =
      wrapper.type === "export_statement" &&
  ...
  private scanVariables(
    ...
    if (context.typeOnly || !context.visible) return;
    const constant = EvidEcmaScriptSyntax.token(declaration, "const");
    for (const declarator of declaration.namedChildren) {
  ...
  private scanNamespace(
    ...
    const names = EvidEcmaScriptSyntax.qualifiedName(nameNode);
    const local = names[0];
    if (local === undefined) {
      this.problem(
  ...
  private scanTypeMembers(
    ...
    for (const member of body.namedChildren) {
      ...
      let symbol: EvidProgrammingSymbol;
      if (member.type === "method_signature") {
        if (
          EvidEcmaScriptSyntax.token(member, "get") ||
          EvidEcmaScriptSyntax.token(member, "set")
        )
          continue;
        symbol = "function";
      ...
      const name = EvidEcmaScriptSyntax.memberName(member);
      if (name === undefined) continue;
      this.addUnit(
        member,
        member,
  ...
  private scanClassMembers(
    ...
    const constructors = body.namedChildren.filter((member) => {
      if (
        ...
        return false;
      return (
        EvidEcmaScriptSyntax.name(member.childForFieldName("name")) ===
        "constructor"
      );
    });
    ...
    if (this.type === "javascript")
      for (const member of body.namedChildren) {
        ...
        const previous: EvidNode | undefined = effective.get(slot);
    ...
    for (const member of body.namedChildren) {
      ...
      if (!EvidEcmaScriptSyntax.publicMember(member)) continue;
      if (method) {
        if (
          EvidEcmaScriptSyntax.token(member, "get") ||
          EvidEcmaScriptSyntax.token(member, "set")
   ...
   * of the Evid symbol inferred from the initializer value.
   */
  private classField(member: EvidNode): boolean {
    return (
      member.type === "public_field_definition" ||
      member.type === "field_definition"
    );
  ...
  private scanParameterProperties(
    constructor: EvidNode,
    owner: string[],
    publicPrefix: string[],
    root: string,
    parentId: string,
    constructors: EvidNode[],
  ): void {
    ...
    for (const parameter of parameters?.namedChildren ?? []) {
      ...
      if (
        ...
        !EvidEcmaScriptSyntax.modifier(parameter, "readonly") &&
        !EvidEcmaScriptSyntax.modifier(parameter, "override")
      )
        continue;
      if (!EvidEcmaScriptSyntax.publicMember(parameter)) continue;
  ...
  private addClassMember(
    ...
    const statically = EvidEcmaScriptSyntax.modifier(member, "static");
    this.addUnit(
      member,
      member,
      symbol,
  ...
  private scanExport(
    ...
    if (EvidEcmaScriptSyntax.token(statement, "=")) {
      this.problem(
    ...
    if (
      EvidEcmaScriptSyntax.token(statement, "as") &&
      EvidEcmaScriptSyntax.token(statement, "namespace")
    ) {
      this.problem(
    ...
    const typeOnly = EvidEcmaScriptSyntax.token(statement, "type");
    const clause = statement.namedChildren.find(
    ...
    if (namespace !== undefined && specifier !== undefined) {
      const publicName = EvidEcmaScriptSyntax.name(
    ...
    if (clause !== undefined) {
      for (const child of clause.namedChildren) {
        ...
        const local = EvidEcmaScriptSyntax.specifierName(child);
        const publicName = EvidEcmaScriptSyntax.specifierAlias(child) ?? local;
        ...
        this.exports.push(
          specifier === undefined
            ? {
                ...
                publicName,
                localName: local,
                typeOnly: typeOnly || EvidEcmaScriptSyntax.token(child, "type"),
              }
            : {
                kind: "named",
                publicName,
    ...
    if (specifier !== undefined)
      ...
      const localName = EvidEcmaScriptSyntax.name(value);
  ...
  private scanJavaScriptDefault(statement: EvidNode, value: EvidNode): void {
    const localName = `default:${statement.startIndex}`;
    const symbol: EvidProgrammingSymbol = EvidEcmaScriptSyntax.functionValue(
      value,
    )
      ? "function"
      : "property";
  ...
  private inspectCommonJsDeclaration(declaration: EvidNode): void {
    for (const declarator of declaration.namedChildren) {
      if (declarator.type !== "variable_declarator") continue;
      const names = EvidEcmaScriptSyntax.bindings(
        declarator.childForFieldName("name"),
      ).flatMap((node) => {
        const name = EvidEcmaScriptSyntax.name(node);
  ...
  private scanCommonJsAssignment(assignment: EvidNode): void {
    const left = assignment.childForFieldName("left");
    const right = assignment.childForFieldName("right");
    if (left === null || right === null) return;
    if (!EvidEcmaScriptSyntax.token(assignment, "=")) {
  ...
  private replaceCommonJs(right: EvidNode, assignment: EvidNode): void {
    ...
    for (const child of right.namedChildren) {
      ...
      if (child.type === "pair") {
        const name = EvidEcmaScriptSyntax.name(child.childForFieldName("key"));
  ...
  private commonJsProperty(node: EvidNode): string | undefined {
    if (node.type !== "member_expression") return undefined;
    const object = node.childForFieldName("object");
    if (!this.exportsObject(object) && !this.moduleExports(object))
      return undefined;
    return EvidEcmaScriptSyntax.name(node.childForFieldName("property"));
  }

  private computedCommonJsProperty(node: EvidNode): boolean {
    if (node.type !== "subscript_expression") return false;
    const object = node.childForFieldName("object");
    return this.exportsObject(object) || this.moduleExports(object);
  }

  private exportsObject(node: EvidNode | null): boolean {
    return node?.type === "identifier" && node.text === "exports";
  }

  private exportsProperty(node: EvidNode): boolean {
    return (
  ...
  private moduleExports(node: EvidNode | null): boolean {
    ...
    return (
      ...
      object.text === "module" &&
      EvidEcmaScriptSyntax.name(property) === "exports"
    );
  }

  private commonJsObject(node: EvidNode): "exports" | "module" | undefined {
    if (this.exportsObject(node)) return "exports";
    return this.moduleExports(node) ? "module" : undefined;
  }

  private containsCommonJsMutation(node: EvidNode, nested: boolean): boolean {
    if (
      nested &&
    ...
    if (
      ...
      EvidEcmaScriptSyntax.token(node, "delete")
    )
      if (
  ...
  private commonJsAssignmentTarget(node: EvidNode): boolean {
    if (
  ...
  private escapesCommonJsObject(node: EvidNode): boolean {
    if (
      node.type === "function_declaration" ||
      node.type === "generator_function_declaration" ||
      node.type === "class_declaration"
  ...
  private attach(
    ...
    while (current !== null && current.type === "comment") {
      if (this.source.content.slice(current.endIndex, boundary).trim() !== "")
        break;
      if (!EvidEcmaScriptSyntax.jsdoc(current)) break;
  ...
  private declaration(statement: EvidNode): EvidNode {
    let declaration =
  ...
  private exportedDeclaration(node: EvidNode | null): boolean {
    return (
      node !== null &&
      (node.type === "class" ||
        node.type === "class_declaration" ||
  ...
  private commentKey(node: EvidNode): string {
