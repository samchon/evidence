 ...
 *
 * The export resolver caches this result for each source and name, then uses its
 * bindings to publish units and its state flags to distinguish exclusions and
 * declaration-free cycles from missing supported declarations.
 */
export interface IEvidEcmaScriptResolution {
   ...
   *
   * Exclusion suppresses a missing-declaration diagnostic and propagates through
   * local and re-export paths.
   */
  excluded: boolean;
   ...
   *
   * A result with no bindings and this flag identifies a declaration-free export
   * cycle that must be reported instead of silently publishing nothing.
   */
  cyclic: boolean;
}
