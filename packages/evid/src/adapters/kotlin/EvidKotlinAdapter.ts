...
import { EvidKotlinDocumentation } from "./EvidKotlinDocumentation";
import { EvidKotlinFileScanner } from "./EvidKotlinFileScanner";
...
export class EvidKotlinAdapter implements IEvidAdapter<"kotlin"> {
  ...
  public async analyze(snapshot: IEvidSourceSnapshot): Promise<IEvidInventory> {
    ...
    try {
      ...
      EvidKotlinReceivers.resolve(analyses);
  ...
  ): Promise<IEvidKotlinFileAnalysis> {
  ...
  private materializeUnits(
    ...
    analyses: IEvidKotlinFileAnalysis[],
  ): Map<string, string> {
  ...
  private materializeUndocumentedHosts(
    inventory: IEvidInventory,
    analysis: IEvidKotlinFileAnalysis,
    published: Map<string, string>,
    hidden: Set<string>,
  ): void {
    ...
    const groups = new Map<string, IEvidKotlinDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidTagParser.parse(
      source.content,
      host,
      EvidKotlinDocumentation.read(source, documentation, host.id),
    );
  }
   ...
   */
  private unitId(declaration: IEvidKotlinDeclaration): string {
