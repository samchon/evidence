...
export interface IEvidKotlinDeclaration {
   ...
   *
   * Receiver resolution and documentation attachments use this scanner-local value before unit materialization assigns a semantic ID.
   */
  id: string;

  /** Gives the literal Kotlin name decoded from the declaration.
   *
   * The scanner combines it with receiver and lexical paths while preserving its source spelling for diagnostics.
   */
  name: string;

  /** Classifies the declaration in Evid's language-independent selector vocabulary.
   *
   * The adapter retains this category while reconciling package-scoped overload families.
   */
  symbol: EvidProgrammingSymbol;

  /** Lists type-parameter names visible at this declaration's lexical scope.
   *
   * Deferred receiver parsing uses these names to reject type-variable receivers that static nominal lookup cannot resolve.
   */
   ...
   *
   * Receiver resolution may replace the extension's lexical owner with its resolved nominal receiver.
   */
  identity: string[];

  /** Lists public accessor segments as projected from the source file.
   *
   * These remain distinct from `identity` so consumers can cite the public spelling of an extension declaration.
   */
  address: string[];

  /** Retains the physical declaration site and content used by hosts and fingerprints.
   *
   * A reconciled overload family can retain several of these source occurrences.
   */
  site: IEvidUnitSite;

  /** States whether this declaration and each containing owner are public.
   *
   * The adapter retains non-public records for receiver lookup but does not publish their units or addresses.
   */
  public: boolean;
   ...
   *
   * Omission denotes a package-level declaration; materialization uses this link to propagate ownership and withdrawals.
   */
