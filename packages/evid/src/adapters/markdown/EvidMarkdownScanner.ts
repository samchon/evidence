 ...
 */
export class EvidMarkdownScanner {
  /**
   ...
   */
  private readonly comments: IEvidMarkdownComment[] = [];
   ...
   */
  private readonly sites = new Map<string, IEvidUnitSite>();
  ...
  private markExamples(): void {
    ...
    for (let index = 0; index < this.lineStarts.length; ++index) {
      ...
      if (fenceMarker !== undefined) {
        ...
        const delimiter: IEvidMarkdownFence | undefined =
          EvidMarkdownSyntax.fence(line);
        if (
          delimiter !== undefined &&
      ...
      if (rendered === undefined && commentStart === undefined) {
        ...
        if (EvidMarkdownSyntax.indentedCode(line)) continue;
      }
      let outside: string = "";
      let cursor: number = 0;
      ...
      while (cursor < line.length) {
        ...
        if (
          kind === "other" ||
          EvidMarkdownSyntax.inlineCode(line, start) ||
          EvidMarkdownSyntax.escaped(line, start)
        ) {
  ...
  private materializeUnits(): void {
    const targetable = this.source.addresses.filter(
      (address) => !EvidMarkdownSyntax.hasWhitespace(address.relative),
    );
    for (const address of this.source.addresses)
      if (EvidMarkdownSyntax.hasWhitespace(address.relative))
    ...
    let hostProblem =
      fileId === undefined
        ...
        : undefined;
    for (let index = 0; index < this.lineStarts.length; ++index) {
      const authored: IEvidMarkdownHeading | undefined =
        this.fenced[index] === true ||
        this.rendered[index] === true ||
        this.commentOnly[index] === true
          ? undefined
          : EvidMarkdownSyntax.heading(this.line(index));
      const heading: IEvidMarkdownHeading | undefined =
        authored === undefined || (this.commentMasks[index]?.length ?? 0) === 0
          ? authored
          : EvidMarkdownSyntax.heading(this.visibleLine(index));
      if (heading !== undefined) {
        hostUnit = undefined;
        hostSite = undefined;
        hostProblem =
          heading.level > 4
            ? `Move the annotation under a Markdown file, H1, H2, H3, or H4 host; H${heading.level} is not an Evid unit.`
        ...
        if (
          ...
          const unit: IEvidUnit = {
            id,
            type: "markdown",
            symbol: EvidMarkdownSyntax.symbol(heading.level),
  ...
  private reportRenderedAnnotations(): void {
    for (let index = 0; index < this.lineStarts.length; ++index) {
      ...
      if (
        ...
        this.rendered[index] === true ||
        this.commentOnly[index] === true ||
        EvidMarkdownSyntax.indentedCode(line)
      )
        continue;
      const marker = EvidMarkdownSyntax.annotation(line);
      ...
      this.inventory.diagnostics.push({
        ...
        severity: "error",
        message: `${marker} is rendered as Markdown prose, so it is not an Evid annotation.`,
