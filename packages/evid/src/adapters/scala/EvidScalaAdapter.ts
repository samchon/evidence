...
import { EvidScalaDocumentation } from "./EvidScalaDocumentation";
import { EvidScalaFileScanner } from "./EvidScalaFileScanner";
...
export class EvidScalaAdapter implements IEvidAdapter<"scala"> {
  ...
  public async analyze(snapshot: IEvidSourceSnapshot): Promise<IEvidInventory> {
    ...
    try {
      ...
      EvidScalaExports.resolve(analyses);
  ...
  ): Promise<IEvidScalaFileAnalysis> {
  ...
  private materializeUnits(
    ...
    analyses: IEvidScalaFileAnalysis[],
  ): Map<string, string> {
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, IEvidScalaDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidTagParser.parse(
      source.content,
      host,
      EvidScalaDocumentation.read(source, documentation, host.id),
    );
  }
