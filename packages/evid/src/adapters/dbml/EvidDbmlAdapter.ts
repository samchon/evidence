...
import { EvidDbmlFileScanner } from "./EvidDbmlFileScanner";
...
export class EvidDbmlAdapter implements IEvidAdapter {
  ...
  private async scan(
    ...
    try {
      return await parser.parse(
        ...
        (session) => new EvidDbmlFileScanner(session, source).scan(),
      );
    } catch (cause) {
      const error = cause instanceof EvidParserError ? cause : undefined;
      return {
        source,
        declarations: [],
        relations: [],
  ...
  private aliases(
    ...
    analyses: IEvidDbmlFileAnalysis[],
  ): Map<string, string[]> {
  ...
  private endpoint(
    ...
    endpoint: IEvidDbmlEndpoint,
    aliases: Map<string, string[]>,
    columns: Set<string>,
  ): IEvidDbmlEndpoint | undefined {
   ...
   */
  private digest(declaration: IEvidDbmlDeclaration, enums: string): string {
