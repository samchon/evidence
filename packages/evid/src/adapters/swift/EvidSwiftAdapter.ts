...
import { EvidSwiftDocumentation } from "./EvidSwiftDocumentation";
import { EvidSwiftFileScanner } from "./EvidSwiftFileScanner";
import { EvidSwiftOwnership } from "./EvidSwiftOwnership";
...
export class EvidSwiftAdapter implements IEvidAdapter {
  ...
  public async analyze(snapshot: IEvidSourceSnapshot): Promise<IEvidInventory> {
    ...
    try {
      ...
      EvidSwiftOwnership.resolve(analyses);
  ...
  ): Promise<IEvidSwiftFileAnalysis> {
  ...
  private materializeUnits(
    ...
    analyses: IEvidSwiftFileAnalysis[],
    moduleRoot: string,
  ): Map<string, string> {
  ...
  private protocolDefault(
    ...
    previous: Set<string>,
    records: Map<string, IEvidSwiftDeclaration>,
  ): boolean {
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, IEvidSwiftDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidTagParser.parse(
      source.content,
      host,
      EvidSwiftDocumentation.read(source, documentation, host.id),
    );
  }
