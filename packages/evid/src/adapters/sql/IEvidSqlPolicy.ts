 ...
 */
export interface IEvidSqlPolicy {
   ...
   */
  validate: (node: EvidNode) => string | undefined;
   ...
   */
  identity?: (address: string[], symbol: EvidDatabaseSymbol) => string[];
   ...
   *
   * Omission uses the scanner's normal relation behavior; `ignore` omits it
   * and `reject` records dialect incompleteness.
   */
