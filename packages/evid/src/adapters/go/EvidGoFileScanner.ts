...
import { EvidGoSyntax } from "./EvidGoSyntax";
...
export class EvidGoFileScanner {
  ...
  public scan(): IEvidGoFileAnalysis {
    ...
    return {
      source: this.source,
      directory: EvidSourcePath.slash(path.dirname(this.source.physicalPath)),
  ...
  private scanTypes(statement: EvidNode): void {
    const specs = statement.namedChildren.filter(
      (node) => node.type === "type_spec" || node.type === "type_alias",
    );
    for (const spec of specs) {
      const name = EvidGoSyntax.name(spec.childForFieldName("name"));
      if (name === undefined || !EvidGoSyntax.exported(name)) continue;
      const form: EvidGoDeclarationForm =
  ...
  private scanStruct(structure: EvidNode, owner: string): void {
    ...
    for (const field of body?.namedChildren ?? []) {
      ...
      const declared = EvidGoSyntax.names(field, "field_identifier");
      const names =
        declared.length !== 0
          ? declared
          : [
      ...
      for (const name of names)
        if (EvidGoSyntax.exported(name))
          this.addDeclaration(field, field, name, "property", "field", owner);
    }
  }

  private scanInterface(contract: EvidNode, owner: string): void {
    for (const member of contract.namedChildren) {
      if (member.type !== "method_elem") continue;
      const name = EvidGoSyntax.name(member.childForFieldName("name"));
      if (name !== undefined && EvidGoSyntax.exported(name))
        this.addDeclaration(
          member,
          member,
          name,
  ...
  private scanFunction(statement: EvidNode): void {
    const name = EvidGoSyntax.name(statement.childForFieldName("name"));
    if (name !== undefined && EvidGoSyntax.exported(name))
      this.addDeclaration(statement, statement, name, "function", "function");
  }

  private scanMethod(statement: EvidNode): void {
    const name = EvidGoSyntax.name(statement.childForFieldName("name"));
    if (name === undefined || !EvidGoSyntax.exported(name)) return;
    const owner = EvidGoSyntax.receiver(statement);
    if (owner === undefined) {
      this.problem(
        "go-receiver",
    ...
    if (!EvidGoSyntax.exported(owner)) return;
    this.addDeclaration(
      statement,
      statement,
      name,
  ...
  private site(node: EvidNode, content: IEvidSourceRange[]): IEvidUnitSite {
    return {
      id: this.siteId(node),
  ...
  private collectLiteralAnnotations(): void {
    ...
    for (const literal of literals) {
      const syntax = EvidGoSyntax.literal(literal);
