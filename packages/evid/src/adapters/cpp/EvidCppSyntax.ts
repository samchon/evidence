...
import type { IEvidCppQualifiedName } from "./IEvidCppQualifiedName";
...
export namespace EvidCppSyntax {
  ...
  export function qualifiedName(
    node: EvidNode | null,
  ): IEvidCppQualifiedName | undefined {
    if (node === null) return undefined;
  ...
  export function templateArity(node: EvidNode): number {
  ...
  export function templateBody(node: EvidNode): EvidNode | undefined {
  ...
  export function topLevelConstObject(node: EvidNode, declarator: EvidNode): boolean {
    ...
    const wrappers: EvidNode[] = [];
    let current: EvidNode | null = declarator;
    while (current !== null) {
      if (
        [
          "pointer_declarator",
  ...
  export function access(node: EvidNode): EvidCppAccess | undefined {
  ...
  function templateParameter(node: EvidNode): boolean {
