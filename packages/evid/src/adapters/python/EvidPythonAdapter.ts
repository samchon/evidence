...
import { EvidPythonExportResolver } from "./EvidPythonExportResolver";
...
export class EvidPythonAdapter implements IEvidAdapter<"python"> {
  ...
  public async analyze(snapshot: IEvidSourceSnapshot): Promise<IEvidInventory> {
    ...
    try {
      ...
      const published = new EvidPythonExportResolver(
        analyses,
        inventory,
        input.root,
      ).publish();
  ...
  ): Promise<IEvidPythonFileAnalysis> {
  ...
  private annotation(
    analysis: IEvidPythonFileAnalysis,
    documentation: IEvidPythonDocumentation,
  ): boolean {
