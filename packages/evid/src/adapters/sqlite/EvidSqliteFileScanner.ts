...
import type { IEvidSqlDocumentation } from "../sql/IEvidSqlDocumentation";
...
export class EvidSqliteFileScanner {
  ...
  private table(node: EvidNode): void {
    ...
    const names = EvidSqliteSyntax.names(node);
    ...
    if (temporary && EvidSqliteSyntax.fold(schema) !== "temp") {
      this.incomplete(node, "A TEMP table may only name the temp schema.");
      return;
    }
    if (EvidSqliteSyntax.fold(name).startsWith("sqlite_")) {
      this.incomplete(
        node,
    ...
    const identity = [schema, name].map(EvidSqliteSyntax.fold);
    ...
    for (const column of columns) {
      ...
      this.declare(
        ...
        "column",
        [...names, columnName],
        [...identity, EvidSqliteSyntax.fold(columnName)],
        model.id,
      );
  ...
  private relation(
    ...
    const remote = clause.namedChildren
      ...
      .flatMap((child) => {
        const name = EvidSqliteSyntax.identifier(child);
    ...
    if (
      local.length === 0 ||
      target === undefined ||
      local.some((name) => !columns.includes(EvidSqliteSyntax.fold(name))) ||
    ...
    this.declare(
      ...
      [...model.identity, EvidSqliteSyntax.fold(segment)],
      model.id,
    );
  }
   ...
   */
  private documentation(): IEvidSqlDocumentation[] {
    const comments = this.session
      .captures("(comment) @comment")
      .map((capture) => capture.node)
      .sort((left, right) => left.startIndex - right.startIndex);
    const groups: EvidNode[][] = [];
   ...
   */
  private leading(node: EvidNode): boolean {
    return (
      this.source.content
        .slice(
   ...
   */
  private incomplete(node: EvidNode, message: string): void {
