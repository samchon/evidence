...
import type { IEvidCDeclarationGroup } from "./IEvidCDeclarationGroup";
import type { IEvidCDocumentation } from "./IEvidCDocumentation";
import type { IEvidCFileAnalysis } from "./IEvidCFileAnalysis";
import { EvidCDocumentation } from "./EvidCDocumentation";
import { EvidCFileScanner } from "./EvidCFileScanner";
...
export class EvidCAdapter implements IEvidAdapter<"c"> {
  ...
  private materializeUnits(
    ...
    const tagForms = new Map<string, Set<EvidCDeclarationForm>>();
    const firstTags = new Map<string, IEvidCDeclaration>();
    for (const declaration of declarations) {
      ...
      const forms = tagForms.get(key) ?? new Set<EvidCDeclarationForm>();
    ...
    const resolvedAddresses = new Map<string, IEvidCDeclarationAddress[]>();
    for (const declaration of declarations)
  ...
  private declarationAddresses(
    declaration: IEvidCDeclaration,
    groups: Map<string, IEvidCDeclarationGroup>,
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, IEvidCDeclaration[]>();
    for (const declaration of analysis.declarations) {
      const unitId = published.get(declaration.id);
      if (
        unitId === undefined ||
  ...
  private parse(
    ...
    return EvidTagParser.parse(
      source.content,
      host,
      EvidCDocumentation.read(source, documentation, host.id),
    );
  }
  ...
  private annotation(
    ...
    return this.annotationPattern(
      EvidCDocumentation.read(analysis.source, documentation, documentation.id)
        .text,
      true,
    );
  }
  ...
  private unitId(declaration: IEvidCDeclaration, sourceId: string): string {
