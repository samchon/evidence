...
import type { IEvidCDocumentationAttachment } from "./IEvidCDocumentationAttachment";

/**
 * Stores a C comment or literal that may carry Evid tags.
 *
 ...
 */
export interface IEvidCDocumentation {
  /**
   * Stable source-range key used to deduplicate one documentation carrier.
   *
   * It identifies the physical carrier rather than a declaration or semantic unit.
   */
  id: string;
   ...
   *
   * The adapter also uses it as the host location for parsed or unsupported annotations.
   */
   ...
   *
   * Documentation reading uses this syntax rather than guessing from raw source.
   */
   ...
   *
   * Unattached tag-bearing carriers remain available for unsupported-host diagnostics.
   */
  attachments: IEvidCDocumentationAttachment[];
}
