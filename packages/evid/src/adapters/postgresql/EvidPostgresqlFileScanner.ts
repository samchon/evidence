 ...
 */
export class EvidPostgresqlFileScanner {
  /**
        ...
        node.namedChildren.some(
          (child) =>
            ...
            (child.type === "identifier" &&
              EvidPostgresqlIdentity.identifier(child.text) === undefined),
        )
      )
        this.problem(
          node,
  ...
  private column(node: EvidNode, table: IEvidSqlDeclaration): void {
    ...
    const name =
      nameNode === null
        ? undefined
        : EvidPostgresqlIdentity.identifier(nameNode.text);
    if (name === undefined) {
      this.problem(
        node,
    ...
    if (
      ...
      const local = (columns?.namedChildren ?? [])
        ...
        .map((child) => {
          ...
          return name === null
            ? undefined
            : EvidPostgresqlIdentity.identifier(name.text);
        });
  ...
  private relation(
    ...
    const identity =
      target === undefined ? undefined : EvidPostgresqlIdentity.path(target);
    const targetIndex =
      target === undefined
        ? -1
    ...
    const remote = node.namedChildren
      ...
      .map((child) => EvidPostgresqlIdentity.identifier(child.text));
    if (
      identity?.length !== 2 ||
    ...
    const name =
      ...
      (node.type === "constraint" && named !== null
        ? EvidPostgresqlIdentity.identifier(named.text)
        : undefined);
    const segment =
      name === undefined
  ...
  private comments(): void {
    ...
    for (let index = 0; index < comments.length; ++index) {
      ...
      const doc: IEvidSqlDocumentation = {
