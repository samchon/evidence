...
export namespace EvidCSharpSyntax {
  ...
  export function typeParameterArity(node: EvidNode): number {
  ...
  export function hasToken(node: EvidNode, token: string): boolean {
    return node.children.some((child) => child.type === token);
  }

  export function accessibility(node: EvidNode): EvidCSharpAccessibility {
  ...
  export function isXmlDocumentation(node: EvidNode): boolean {
    return (
      node.type === "comment" &&
  ...
  export function explicitInterface(node: EvidNode): boolean {
    return node.namedChildren.some(
      (child) => child.type === "explicit_interface_specifier",
    );
  }

  export function operatorName(node: EvidNode): string | undefined {
  ...
  export function conversionOperatorName(node: EvidNode): string | undefined {
