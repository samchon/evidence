...
/**
 * Records one physical C# declaration before adapter-level family reconciliation.
 *
 * `EvidCSharpFileScanner` creates this record with the declaration's syntax-derived
 * identity, addresses, visibility inputs, and documentation site. `EvidCSharpAdapterBase`
 * groups compatible records into semantic units, so partial declarations and
 * overloads can retain their distinct source attachments while sharing one unit.
 */
export interface IEvidCSharpDeclaration {
  ...
  /** Canonical semantic path independent of aliases and physical source location.
   *
  ...
  /** Source location retained as an eligible documentation host and fingerprint site.
   *
