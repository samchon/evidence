...
/**
 * Carries the enclosing C# type state while the scanner traverses nested declarations.
 *
 * EvidCSharpFileScanner creates this context for each type and passes it to member
 * and nested-type scans. It keeps structural ownership, semantic identity, and
 * public address candidates aligned without repeatedly deriving them from syntax.
 */
export interface IEvidCSharpTypeContext {
   ...
   *
   * The scanner appends each nested declaration name to this canonical identity.
   * It remains separate from public addresses because aliases may expose other
   * valid access paths.
   */
  identity: string[];
   ...
   * semantics without reinterpreting the enclosing type's syntax for each member.
   */
