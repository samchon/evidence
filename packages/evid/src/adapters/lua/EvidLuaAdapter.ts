...
import { EvidLuaDocumentation } from "./EvidLuaDocumentation";
 ...
 */
export class EvidLuaAdapter implements IEvidAdapter<"lua"> {
  /**
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, IEvidLuaDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidTagParser.parse(
      source.content,
      host,
      EvidLuaDocumentation.read(source, documentation, host.id),
    );
  }

  /**
   * Detects Evid and withdrawal annotations outside masked examples.
   *
