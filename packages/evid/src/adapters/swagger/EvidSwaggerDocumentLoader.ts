...
import type { IEvidSwaggerCacheEntry } from "./IEvidSwaggerCacheEntry";
...
import type { EvidSwaggerDocumentInput } from "./EvidSwaggerDocumentInput";
import { EvidYamlScalarMapper } from "./EvidYamlScalarMapper";
...
const cache = new Map<string, IEvidSwaggerCacheEntry>();
 ...
 */
export namespace EvidSwaggerDocumentLoader {
  /**
  ...
  export async function load(
  ...
  ): Promise<IEvidSwaggerLoadResult> {
    ...
    try {
      ...
      const document: OpenApi.IDocument = OpenApiConverter.upgradeDocument(
        typia.assert<EvidSwaggerDocumentInput>(input),
      );
      const result: IEvidSwaggerLoadResult = {
...
function semanticOperation(
  ...
  const output: Record<string, unknown> = EvidCanonicalJson.without(operation, [
    "description",
  ]);
  ...
  if (operation.description !== undefined) {
    const description: string = EvidSwaggerDescription.semantic(
      operation.description,
    );
 ...
 * needed for Evid tags; unavailable parser ranges leave location optional.
 */
function location(
  content: string,
  yaml: EvidYamlDocument.Parsed<EvidYamlParsedNode>,
...
): IEvidSwaggerOperationLocation | undefined {
  ...
  if (description !== undefined) {
    ...
    if (isScalar<string>(scalar) && typeof scalar.value === "string")
      mapping = EvidYamlScalarMapper.map(content, scalar);
  }
  return {
