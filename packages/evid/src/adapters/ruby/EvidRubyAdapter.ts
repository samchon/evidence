...
export class EvidRubyAdapter implements IEvidAdapter<"ruby"> {
  ...
  private materializeUnits(
    ...
    analyses: IEvidRubyFileAnalysis[],
  ): Map<string, string> {
    const declarationAnalysis = new Map<string, IEvidRubyFileAnalysis>();
    const groups = new Map<string, IEvidRubyDeclaration[]>();
    for (const analysis of analyses)
  ...
  private checkGroup(
    ...
    declarationAnalysis: Map<string, IEvidRubyFileAnalysis>,
  ): void {
    const first = declarations[0];
  ...
  private checkAddressCollisions(
    ...
    const firstDeclarations = new Map<string, IEvidRubyDeclaration>();
    const declarationAnalysis = new Map<string, IEvidRubyFileAnalysis>();
    for (const analysis of analyses)
      for (const declaration of analysis.declarations) {
        const unitId = published.get(declaration.id);
  ...
  private materializeUndocumentedHosts(
    ...
    for (const analysis of analyses) {
      ...
      const sites = new Map<string, IEvidRubyDeclaration[]>();
      for (const declaration of analysis.declarations) {
        const unitId = published.get(declaration.id);
        if (
          unitId === undefined ||
  ...
  private parse(
    ...
    return EvidTagParser.parse(source.content, host, {
      ...mapping,
      hostId: host.id,
    });
  }
  ...
  private annotation(
    analysis: IEvidRubyFileAnalysis,
    documentation: IEvidRubyDocumentation,
  ): boolean {
  ...
  private addSite(unit: IEvidUnit, candidate: IEvidUnitSite): void {
  ...
  private unitId(declaration: IEvidRubyDeclaration): string {
  ...
  private problem(
    inventory: IEvidInventory,
    analysis: IEvidRubyFileAnalysis | undefined,
    declaration: IEvidRubyDeclaration,
    code: string,
    message: string,
    repair: string,
  ): void {
