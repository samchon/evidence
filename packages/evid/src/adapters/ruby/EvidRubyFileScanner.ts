...
import type { IEvidRubyFileAnalysis } from "./IEvidRubyFileAnalysis";
import type { IEvidRubyScopeContext } from "./IEvidRubyScopeContext";
...
import { EvidRubySyntax } from "./EvidRubySyntax";
...
export class EvidRubyFileScanner {
   ...
   */
  private readonly declarations: IEvidRubyDeclaration[] = [];
   ...
   */
  private readonly documentation = new Map<string, IEvidRubyDocumentation>();
   ...
   */
  private readonly constantVisibility = new Map<string, EvidRubyVisibility>();
   ...
   */
  public scan(): IEvidRubyFileAnalysis {
    const root: IEvidRubyScopeContext = {
      kind: "top",
      identity: [],
      side: "instance",
      visibility: "private",
   ...
   */
  private scanBody(body: EvidNode, context: IEvidRubyScopeContext): void {
  ...
  private scanContainer(
    ...
    this.addDeclaration(
      ...
      undefined,
      kind,
      kind === "class" ? EvidRubySyntax.superclass(statement) : undefined,
    );
    const body = statement.childForFieldName("body");
    if (body === null) return;
    const nested: IEvidRubyScopeContext = {
      kind,
      identity,
      side: "instance",
      visibility: "public",
  ...
  private scanSingletonClass(
    ...
    const identity =
      value?.type === "self" && context.kind !== "top"
        ? context.identity
        : this.resolveOptionalPath(EvidRubySyntax.constantPath(value), context);
    ...
    const body = statement.childForFieldName("body");
    if (body === null) return;
    const singleton: IEvidRubyScopeContext = {
      kind: "singleton",
      identity,
      side: "singleton",
      visibility: "public",
  ...
  private scanMethod(
    ...
    wrapper: EvidNode,
    context: IEvidRubyScopeContext,
    forcedVisibility?: EvidRubyVisibility,
    forcedModuleFunction: boolean = false,
  ): void {
    if (context.kind === "top") return;
    const name = EvidRubySyntax.methodName(method.childForFieldName("name"));
    if (name === undefined) {
      this.problem(
        "ruby-method-name",
  ...
  private scanSingletonMethod(
    ...
    const owner: IEvidRubyScopeContext = {
      kind: "singleton",
      identity,
      side: "singleton",
  ...
  private addMethod(
    ...
    form: Extract<EvidRubyDeclarationForm, "method" | "singleton-method">,
  ): IEvidRubyDeclaration {
  ...
  private scanAssignment(statement: EvidNode, context: IEvidRubyScopeContext): void {
    ...
    if (EvidRubySyntax.generatedConstant(statement.childForFieldName("right")))
      this.problem(
   ...
   */
  private scanCall(call: EvidNode, context: IEvidRubyScopeContext): void {
    const name = EvidRubySyntax.callName(call);
    if (name === undefined) return;
    if (!EvidRubySyntax.hasReceiver(call)) {
  ...
  private scanVisibilityCall(
    ...
    const arguments_ = EvidRubySyntax.callArguments(call);
    ...
    if (arguments_.length === 1 && wrapped !== undefined) {
      ...
      if (
        ...
        ["attr_reader", "attr_writer", "attr_accessor"].includes(
          EvidRubySyntax.callName(wrapped) ?? "",
        )
      ) {
    ...
    const names = EvidRubySyntax.literalNames(arguments_);
  ...
  private scanClassVisibilityCall(
    call: EvidNode,
    context: IEvidRubyScopeContext,
    visibility: Extract<EvidRubyVisibility, "private" | "public">,
  ): void {
  ...
  private scanAttribute(
    ...
    if (context.kind === "top") return;
    const callName = EvidRubySyntax.callName(call);
    if (
    ...
    const names = arguments_.map((argument) =>
      EvidRubySyntax.literalName(argument),
    );
    if (
      names.length === 0 ||
    ...
    const modes: EvidRubyAttributeMode[] =
      callName === "attr_reader"
        ? ["read"]
        : callName === "attr_writer"
          ? ["write"]
   ...
   */
  private cloneModuleFunction(target: IEvidRubyDeclaration): void {
    const address = this.memberAddress(
      target.ownerIdentity ?? [],
      "singleton",
      target.runtimeName,
    );
    const clone: IEvidRubyDeclaration = {
      ...target,
   ...
   * source region Evid associates with the declaration.
   */
  private addDeclaration(
    ...
    const declaration: IEvidRubyDeclaration = {
   ...
   */
  private lineLeading(range: IEvidSourceRange): boolean {
    const lineStart =
      Math.max(
  ...
  private collectDocumentation(): void {
    ...
    for (let index = 0; index < comments.length; ++index) {
      ...
      const syntax = EvidRubySyntax.commentSyntax(first);
   ...
   *
   * These literal carriers support Evid annotations even without comments.
   */
  private collectLiteralAnnotations(): void {
    ...
    for (const content of contents) {
      ...
      this.documentation.set(id, {
        id,
        range,
        mapping: EvidDocumentation.read(this.source.content, id, range, {
          opening: "",
          closing: "",
          tagBoundaries: true,
   ...
   */
  private containsSurfaceChange(node: EvidNode): boolean {
   ...
   */
  private dynamicDirective(node: EvidNode, description: string): void {
    this.problem(
  ...
  /**
   * Checks whether raw Ruby carrier text contains an Evid annotation tag.
   *
