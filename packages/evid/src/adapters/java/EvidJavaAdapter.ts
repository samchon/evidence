...
import { EvidJavaDocumentation } from "./EvidJavaDocumentation";
import { EvidJavaFileScanner } from "./EvidJavaFileScanner";
 ...
 * state.
 */
export class EvidJavaAdapter implements IEvidAdapter<"java"> {
  /**
  ...
  private materializeUnits(
    ...
    analyses: IEvidJavaFileAnalysis[],
  ): Map<string, string> {
  ...
  private materializeUndocumentedHosts(
    inventory: IEvidInventory,
    analysis: IEvidJavaFileAnalysis,
    published: Map<string, string>,
    hidden: Set<string>,
  ): void {
    ...
    const groups = new Map<string, IEvidJavaDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidTagParser.parse(
      source.content,
      host,
      EvidJavaDocumentation.read(source, documentation, host.id),
    );
  }
  ...
  private unitId(declaration: IEvidJavaDeclaration): string {
