...
import type { IEvidRustPublicOccurrence } from "./IEvidRustPublicOccurrence";
import type { IEvidRustResolvedMember } from "./IEvidRustResolvedMember";
...
export class EvidRustModuleResolver {
  ...
  private readonly exports = new Map<
    string,
    Map<string, IEvidRustExportRecord[]>
  >();
  private readonly externalTargets = new Map<string, IEvidRustFileAnalysis>();
  private readonly identities = new Map<string, string>();
  private readonly implMembers = new Map<string, IEvidRustResolvedMember[]>();
  private readonly modules = new Map<string, IEvidRustModuleRecord>();
  private readonly placements = new Map<string, IEvidRustFilePlacement>();
   ...
   */
  public constructor(
    private readonly analyses: IEvidRustFileAnalysis[],
  ...
  private resolveExternalTargets(): void {
    const sources = new Map<string, IEvidRustFileAnalysis[]>();
  ...
  private placeFiles(): void {
    ...
    const queue: IEvidRustFilePlacement[] = [];
    for (const analysis of this.analyses)
  ...
  private attachExternalInnerDocumentation(): void {
    const sourceDeclarations = new Map<string, IEvidRustDeclaration>();
    for (const analysis of this.analyses)
  ...
  private buildModules(): void {
    ...
    const rawDeclarations = new Map<string, IEvidRustDeclaration>();
    for (const analysis of this.analyses)
      for (const declaration of analysis.declarations)
    ...
    for (const placement of this.placements.values())
      for (const declaration of placement.analysis.declarations) {
        ...
        if (
          ...
          const located: IEvidRustLocatedDeclaration = {
            declaration,
            placement,
            module,
  ...
  private buildExports(): void {
    for (const module of this.modules.values()) {
      const entries = new Map<string, IEvidRustExportRecord[]>();
      ...
      for (const implementation of placement.analysis.implementations) {
        ...
        if (
          ...
          this.implProblem(
            ...
            "Limit Evid ownership to an impl for one selected local struct or enum.",
          );
          continue;
        }
        ...
        for (const member of members) {
          ...
          const resolved: IEvidRustResolvedMember = {
            declaration: located,
            owner,
            ...(qualifier === undefined ? {} : { qualifier }),
            ...(trait[0] === undefined
  ...
  private collectOccurrences(): IEvidRustPublicOccurrence[] {
    const output: IEvidRustPublicOccurrence[] = [];
  ...
  private collectModule(
    ...
    output: IEvidRustPublicOccurrence[],
  ): void {
    const entries = this.exports.get(module.key);
  ...
  private materialize(occurrences: IEvidRustPublicOccurrence[]): void {
  ...
  private materializeOwnedMembers(
    occurrence: IEvidRustPublicOccurrence,
    ownerUnit: IEvidUnit,
    reachable: Set<string>,
    roots: Map<string, IEvidRustFilePlacement>,
  ): void {
  ...
  private materializeMember(
    occurrence: IEvidRustPublicOccurrence,
    ownerUnit: IEvidUnit,
    member: IEvidRustLocatedDeclaration,
    suffix: string[],
    root: IEvidRustFilePlacement | undefined,
  ): void {
  ...
  private unitId(located: IEvidRustLocatedDeclaration): string {
  ...
  private resolveModulePath(
    module: IEvidRustModuleRecord,
    sourcePath: string[],
  ): IEvidRustModuleRecord | undefined {
  ...
  private addExport(
    entries: Map<string, IEvidRustExportRecord[]>,
    record: IEvidRustExportRecord,
  ): boolean {
  ...
  private uniqueLocated(
    records: IEvidRustLocatedDeclaration[],
  ): IEvidRustLocatedDeclaration[] {
    return Array.from(
      new Map(
        records.map((record) => [record.declaration.id, record]),
  ...
  private sourceLocations(analysis: IEvidRustFileAnalysis): string[] {
    return Array.from(
      new Set([
        EvidSourcePath.slash(analysis.source.physicalPath),
        ...analysis.source.addresses.map((address) =>
          EvidSourcePath.slash(address.absolute),
        ),
      ]),
    );
  }
  ...
  private implProblem(
    implementation: IEvidRustImplementation,
  ...
  private problem(
    code: string,
    analysis: IEvidRustFileAnalysis | undefined,
    message: string,
    repair: string,
    key: string,
  ): void {
