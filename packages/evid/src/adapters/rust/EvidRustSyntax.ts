...
export namespace EvidRustSyntax {
   ...
   */
  export function visibility(node: EvidNode): EvidRustVisibility {
   ...
   */
  export function typeParameters(node: EvidNode): string[] {
   ...
   */
  export function ordinaryComment(node: EvidNode): boolean {
    return (
   ...
   */
  export function attributeValue(node: EvidNode): EvidNode | null {
    const attribute =
      node.type === "attribute_item" || node.type === "inner_attribute_item"
        ? (node.namedChildren.find((child) => child.type === "attribute") ??
          null)
   ...
   */
  export function outerDocumentation(node: EvidNode): boolean {
   ...
   *
   * EvidRustFileScanner uses the returned syntax to map text and preserve tag
  ...
  function isOuterDocumentation(node: EvidNode): boolean {
    return (
      (node.type === "line_comment" || node.type === "block_comment") &&
      node.childForFieldName("outer") !== null
    );
