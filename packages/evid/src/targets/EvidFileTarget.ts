...
/**
 * Converts file-qualified Evid targets between authored text and addresses.
 *
...
export namespace EvidFileTarget {
  ...
  export function parse(target: string, origin: string): IEvidAddress {
    ...
    return {
      ...
      segments: hash < 0 ? [] : EvidAccessor.parse(target.slice(hash + 1)),
    };
  }
  ...
  export function format(address: IEvidAddress): string {
    ...
    return address.segments.length === 0
      ? encoded
      : encoded + "#" + EvidAccessor.format(address.segments);
  }
