...
import { EvidTargetBody } from "../internal/EvidTargetBody";
import { EvidDocumentationExamples } from "../parsers/EvidDocumentationExamples";
import type { IEvidDocumentationFence } from "../parsers/IEvidDocumentationFence";
...
export namespace EvidTagProgrammer {
  /**
   * Parses valid Evid tags and diagnostics from one mapped documentation block.
   *
   * The function consumes no text outside {@link IEvidTagContext.documentation}.
   ...
   */
  export function parse(context: IEvidTagContext): IEvidTagParseResult {
    validate(context);
    const characters: string[] = context.documentation.text.split("");
    EvidDocumentationExamples.maskHtmlComments(
      characters,
      context.documentation.text,
    );
    const input: string = characters.join("");
    const lines: string[] = input.split("\n");
    const documentBaseline: number = EvidDocumentationExamples.baseline(input);
    let cursor = 0;
    for (const rawLine of lines) {
      ...
      const delimiter: IEvidDocumentationFence | undefined =
        EvidDocumentationExamples.fence(rawLine, documentBaseline);
  ...
  function flush(context: IEvidTagContext): void {
    ...
    const body = EvidTargetBody.split(tag.body);
    try {
      EvidTargetBody.check(body.target, tag.kind === "link");
    } catch (cause) {
      problem(
        context,
        "malformed-target",
