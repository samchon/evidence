...
import type { IEvidGraphBoundary } from "../structures/IEvidGraphBoundary";
import type { IEvidGraphExportEdge } from "../structures/IEvidGraphExportEdge";
import type { IEvidGraphExportReview } from "../structures/IEvidGraphExportReview";
...
import type { IEvidGraphPolicy } from "../structures/IEvidGraphPolicy";
...
import type { IEvidInspectedAcknowledgement } from "../structures/IEvidInspectedAcknowledgement";
import type { IEvidInspectedObligation } from "../structures/IEvidInspectedObligation";
import type { IEvidInspectedReview } from "../structures/IEvidInspectedReview";
import type { IEvidInspectedUnit } from "../structures/IEvidInspectedUnit";
...
import type { IEvidReview } from "../structures/IEvidReview";
import type { IEvidSourceLocation } from "../structures/IEvidSourceLocation";
 ...
 *
 * @example
 *   const report = EvidQueryProgrammer.list(queryContext, "typescript");
 *   const inspection = await EvidQueryProgrammer.inspect(
 *     queryContext,
...
export namespace EvidQueryProgrammer {
  ...
  export async function inspect(
    ...
    const exitCode: EvidCommandExitCode = incomplete
      ? 2
      : !resolved
        ? 1
   ...
   */
  export function graph(context: IEvidQueryContext): IEvidGraphReport {
    const { analysis } = context;
    const root = context.cwd;
    const configured = context.populations;
    const nodes = new Map<string, EvidGraphNode>();
    const boundaries: IEvidGraphBoundary[] = [];
    const edges: IEvidGraphExportEdge[] = [];
    const reviews: IEvidGraphExportReview[] = [];
  ...
  export function languages(): IEvidLanguagesReport {
    const languages = [
      ...EvidLanguageRegistry.list().flatMap((language) =>
      ...
      ...EvidLanguageRegistry.databases().flatMap((language) =>
        language.adapter === undefined
          ? []
          : [
              {
  ...
  export function populations(
  ...
  ): EvidQueryPopulationContext[] {
    const output: IEvidQueryPopulation[] = [];
    analysis.graphInput.claims.forEach((claim, claimPosition) => {
      const reportClaim = requireReportClaim(analysis, claimPosition);
      output.push({
        scope: {
    ...
    return output.map(
      (population) => new EvidQueryPopulationContext(population),
    );
  }
  ...
  async function inspectPopulation(
    ...
    cwd: string,
    target: string,
  ): Promise<IEvidInspection> {
    const host = commandHost(cwd);
    const statement: IEvidTargetStatement = {
      target,
      hostId: host.id,
      location: { file: host.file, range: host.range },
    };
    const resolution = await new EvidTargetResolver(
  ...
  function inspectedUnit(
    ...
    const item = listItem(population, unit, cwd);
    if (item === undefined)
      throw new Error(`Evid identity '${unit.id}' has no public address.`);
  ...
  function applicablePopulations(
    populations: EvidQueryPopulationContext[],
    target: string,
  ): EvidQueryPopulationContext[] {
    ...
    for (const language of [
      ...
      try {
        EvidLanguageRegistry.select(language.type, file);
  ...
  function formatAddress(
    ...
    if (population.scope.type === "markdown") {
      ...
      return files.map((candidate) =>
        segments.length === 0
          ? EvidMarkdownTarget.normalize(candidate)
          : `${EvidMarkdownTarget.normalize(candidate)}#${segments.join("#")}`,
      );
    }
    return [
      EvidFileTarget.format({
  ...
  function graphPolicy(
    ...
    return {
      ...
      uniqueEvid: reference.uniqueEvid === true,
      singleEvidPerSymbol: reference.singleEvidPerSymbol === true,
  ...
  function requireReportClaim(
    analysis: IEvidCheckAnalysis,
    position: number,
  ): IEvidCheckAnalysis["report"]["claims"][number] {
  ...
  function requireResultClaim(
    analysis: IEvidCheckAnalysis,
    position: number,
  ): IEvidCheckAnalysis["graph"]["claims"][number] {
  ...
  function requireReportObligation(
    ...
    claim: number,
    reference: number,
  ): IEvidCheckAnalysis["report"]["claims"][number]["obligations"][number] {
  ...
  function requireResultObligation(
    ...
    claim: number,
    reference: number,
  ): IEvidCheckAnalysis["graph"]["claims"][number]["obligations"][number] {
   ...
   */
  function requireHost(hosts: IEvidHost[], id: string): IEvidHost {
