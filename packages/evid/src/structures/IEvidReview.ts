 ...
 * acknowledgement kind. Required reviews must also carry the current fingerprint
 * of that target and its structural subtree. Matching target text alone cannot
 * compensate for an unrelated host or the wrong review kind.
 *
 * Reviews remain separate from acknowledgements because they never discharge
 * coverage. Missing fingerprints are retained for diagnostics, and semantic edits
 * can expire a previously valid review. The checker validates this record and
...
export interface IEvidReview extends IEvidTargetStatement {
   ...
   * from the acknowledgement being reviewed, even when host and target coincide.
   */
  id: string;
   ...
   *
   * Positive evidence needs a positive review; exclusions need exclusion reviews.
   * The wrong kind produces a pairing finding rather than satisfying review policy.
   */
   ...
   *
   * Omission remains a parseable review state. A reference requiring review reports
   * a missing token or a stale token against the current target fingerprint.
   */
  fingerprint?: string;
   ...
   *
   * Tag parsing requires explanatory text. Graph policy checks pairing and content
   * freshness without judging whether the described verification actually occurred.
   */
  description: string;
}
