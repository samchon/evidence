import type { IEvidSourceAddress } from "./IEvidSourceAddress";
import type { IEvidSourceFingerprintRoot } from "./IEvidSourceFingerprintRoot";
 ...
 */
export interface IEvidSourceFile {
  /**
   * Filesystem identity used to deduplicate the captured source.
   *
   * Discovery uses device/inode identity where available and falls back to
   * realpath. Logical aliases and hard links can therefore share this ID
   * without losing paths.
   */
  id: string;
   ...
   *
   * This path anchors physical declaration sites. Hard-linked paths may share
   * the same source ID, while logical citation paths remain in `addresses`.
   */
  physicalPath: string;
   ...
   *
   * Filesystem discovery selects a deterministic logical address relative to
   * the configuration directory. It stays independent of device/inode identity
   * and canonical link targets while still distinguishing separate declaring
   * files.
   */
   ...
   */
  fingerprintRoot?: IEvidSourceFingerprintRoot;
   ...
   * Normalize only where a later semantic digest requires it, not before
   * extraction.
   */
  content: string;
   ...
   *
   * This detects source changes, including edits irrelevant to one unit's
   * semantic fingerprint. Review tokens are constructed separately from unit
   * content.
   */
  digest: string;
   ...
   * Address metadata distinguishes selection spelling from the canonical read
   * path.
   */
  addresses: IEvidSourceAddress[];
}
