 ...
 * filtered by artifact family and symbol kind. Filters affect displayed rows only:
 * completeness, success, and diagnostics still describe the underlying full check.
 */
export interface IEvidListReport {
   ...
   *
   * Filtering away rows cannot turn incomplete extraction into a complete result.
   */
   ...
   *
   * Discovering usable targets does not suppress coverage errors elsewhere in that check.
   */
  success: boolean;
   ...
   *
   * The list operation preserves incomplete and error states despite returning rows.
   */
   ...
   *
   * This counts population-qualified rows, including structural ancestors, rather
   * than reporting the check's coverage denominator.
   */
  total: number;
   ...
   *
   * Each row retains its aliases and selection state for authoring exact citations.
   */
  items: IEvidListItem[];
   ...
   *
   * They remain visible even if language or kind filters omit the affected rows.
   */
