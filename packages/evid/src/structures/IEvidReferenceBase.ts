 ...
 *
 * 1. `noEvidExclude` requires positive evidence rather than non-applicability.
 * 2. `uniqueEvid` limits positive semantic hosts for each selected unit.
 * 3. `singleEvidPerSymbol` requires one selected target per claim host.
 ...
 */
export interface IEvidReferenceBase<
  Type extends string,
...
> {
  ...
  /**
   * Evid symbol kinds selected as required units.
   *
   ...
   *
   * - Evid needs evidenceReview; exclusions need evidenceExcludeReview. Reviews
