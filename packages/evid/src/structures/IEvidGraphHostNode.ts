...
/**
 * Annotation-carrier node used when a relationship has no semantic claim endpoint.
 *
 * Some eligible carriers, including file-level exclusions, have no selected claim
 * unit. Export retains their physical host as an endpoint so the relationship does
 * not disappear or invent a semantic declaration solely for visualization.
 */
export interface IEvidGraphHostNode {
   ...
   *
   * Another reference can represent the same physical host under a different node ID.
   */
  boundaryId: string;
   ...
   *
   * The current exporter uses the host identity because no declaration name exists.
   */
  name: string;
   ...
   *
   * This provides a repair location despite the absence of a semantic claim unit.
   */
