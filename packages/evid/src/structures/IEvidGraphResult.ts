...
import type { IEvidGraphClaimResult } from "./IEvidGraphClaimResult";
 ...
 * remain. Merely obtaining an empty missing-unit list from partial input does not
 * establish success.
 */
export interface IEvidGraphResult {
  /**
   * Whether all active evaluation boundaries are complete and finding-free.
   *
   * Inactive claims do not make the graph incomplete. Any retained diagnostic or
   * active incomplete obligation prevents this flag from becoming true.
   */
  success: boolean;
   ...
   * references are preserved rather than merged into one aggregate coverage set.
   */
  claims: IEvidGraphClaimResult[];

  /**
   * Deduplicated findings from extraction integrity and policy evaluation.
   *
   * Finalization includes findings that depend on cross-reference participation.
   * Consumers can render them without rerunning the graph traversal.
   */
