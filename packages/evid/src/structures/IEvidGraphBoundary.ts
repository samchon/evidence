 ...
 * Selected unit IDs establish its denominator; checklist mode additionally
 * keeps each claim host's answer ledger so answers cannot be pooled between
 * hosts.
 */
export interface IEvidGraphBoundary {
   ...
   *
   * Related nodes and relationships use this key to retain obligation
   * ownership.
   */
  id: string;
   ...
   *
   * Its role, artifact type, and authored index describe the citing side of the
   * pair.
   */
   ...
   *
   * Its reference index distinguishes repeated requirements under the same
   * claim.
   */
   ...
   *
   * Inherited settings have been resolved into the booleans and severity used
   * by evaluation.
   */
   ...
   *
   * Inactive boundaries retain context without contributing active
   * requirements.
   */
  active: boolean;
   ...
   *
   * These retain the obligation's uncovered requirements rather than unresolved
   * names.
   */
   ...
   *
   * Ordinary coverage leaves the array empty; checklist answers remain
   * attributable to each semantic claim subject.
   */
