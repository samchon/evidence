...
export namespace EvidGraphReporter {
   ...
   */
  export function mermaid(report: IEvidGraphReport): string {
  ...
  export function dot(report: IEvidGraphReport): string {
    const names = nodeNames(report.nodes);
    const lines: string[] = ["digraph Evid {", "  rankdir=LR;"];
    report.boundaries.forEach((entry, index) => {
      lines.push(
 ...
 */
function nodeNames(nodes: EvidGraphNode[]): Map<string, string> {
...
function boundaryLabel(boundary: IEvidGraphBoundary): string {
  const policies = [
    boundary.policy.noEvidExclude ? "no-exclude" : undefined,
    boundary.policy.uniqueEvid ? "unique" : undefined,
    boundary.policy.singleEvidPerSymbol ? "single" : undefined,
 ...
 */
function edgeLabel(edge: IEvidGraphExportEdge): string {
