 ...
 *
 * The graph creates this context after materialized claim targets are reconciled.
 * Every reference beneath the claim reuses its host population, but resolves and
 * evaluates reference-side units independently through its own context.
 */
export interface IEvidGraphClaimContext {
  /**
   ...
   */
  readonly claim: IEvidGraphClaim;

  /**
   * Reconciled claim inventory used for diagnostics and host lookup.
   *
   * Semantic validation failures are retained here so graph evaluation can report
   * an incomplete claim instead of evaluating only the records that survived.
   */
   ...
   *
   * Structural ancestors make aggregate addresses resolvable, whereas the selected
   * set determines required claim membership and the available attachment hosts.
   */
