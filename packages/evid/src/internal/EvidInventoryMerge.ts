...
import { EvidInventorySources } from "./EvidInventorySources";
 ...
 */
export namespace EvidInventoryMerge {
  /**
   ...
   * population.
   */
  export function combine(inputs: IEvidInventory[]): IEvidInventory {
    EvidInventorySources.reconcile(inputs);
    const output: IEvidInventory = {
      schemaVersion: 1,
      sources: [],
      annotationRanges: [],
      units: [],
    ...
    const sources = new Map<string, IEvidSourceFile>();
    const units = new Map<string, IEvidUnit>();
    const hosts = new Map<string, IEvidHost>();
  ...
  export function sourceAddresses(
    values: IEvidSourceAddress[],
  ): IEvidSourceAddress[] {
    const records = new Map<string, IEvidSourceAddress>();
   ...
   * extraction order.
   */
  export function siteKey(site: IEvidUnitSite): string {
   ...
   */
  export function contentKey(site: IEvidUnitSite): string {
    return JSON.stringify(
      unique(
   ...
   */
  function hostKey(host: IEvidHost): string {
    return JSON.stringify([
      host.file,
   ...
   */
  function conflict(output: IEvidInventory, kind: string, id: string): void {
 ...
 */
function fingerprintRootKey(source: IEvidSourceFile): string {
