...
import type { IEvidGrammarAsset } from "../structures/IEvidGrammarAsset";
import type { IEvidTreeSitterAssetLock } from "./IEvidTreeSitterAssetLock";
...
export class EvidTreeSitterAssetCache {
  ...
  public async bytes(grammar: IEvidGrammar): Promise<Uint8Array> {
    ...
    try {
      ...
      let pending = EvidTreeSitterAssetCache.pending.get(destination);
      if (pending === undefined || pending.controller.signal.aborted) {
        ...
        EvidTreeSitterAssetCache.pending.set(destination, pending);
      }
      ++pending.consumers;
      try {
        ...
        if (--pending.consumers === 0) {
          ...
          if (EvidTreeSitterAssetCache.pending.get(destination) === pending)
            EvidTreeSitterAssetCache.pending.delete(destination);
        }
      }
    } catch (cause) {
  ...
  private async acquire(
    ...
    const owner: IEvidTreeSitterAssetLock = {
      pid: process.pid,
      token: randomUUID(),
    };
 ...
 */
function matches(bytes: Uint8Array, asset: IEvidGrammarAsset): boolean {
  return (
...
async function readLock(
  file: string,
): Promise<IEvidTreeSitterAssetLock | undefined> {
  try {
    const parsed = typia.json.validateParse<IEvidTreeSitterAssetLock>(
