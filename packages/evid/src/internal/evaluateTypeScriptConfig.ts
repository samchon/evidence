...
import type { IEvidEvaluateTypeScriptConfigOptions } from "./IEvidEvaluateTypeScriptConfigOptions";
...
export async function evaluateTypeScriptConfig(
  configFile: string,
  options: IEvidEvaluateTypeScriptConfigOptions = {},
): Promise<unknown> {
  ...
  try {
    ...
    const manifest = typia.json.assertParse<IEvidTtsxManifest>(
...
async function configModule(
  ...
  for (;;) {
    ...
    if (await exists(file)) {
      const scope = typia.json.assertParse<IEvidConfigPackageScope>(
