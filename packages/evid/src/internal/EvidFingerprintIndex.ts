 ...
 */
export class EvidFingerprintIndex {
  /**
   ...
   */
  private readonly annotations = new Map<string, IEvidSourceRange[]>();
   ...
   */
  private readonly children = new Map<string, IEvidUnit[]>();
   ...
   */
  private readonly fingerprints = new Map<string, IEvidFingerprint>();
   ...
   */
  private readonly sources = new Map<string, IEvidSourceFile>();
   ...
   */
  public constructor(inventory: IEvidInventory) {
   ...
   */
  public inspect(unitId: string): IEvidFingerprint {
   ...
   */
  private identity(unit: IEvidUnit): string {
  ...
  private portableUnitId(unit: IEvidUnit): string {
    ...
    const replacements: EvidPortableReplacement[] = [];
   ...
   */
  private collect(root: IEvidUnit): IEvidUnit[] {
    const output: IEvidUnit[] = [];
    const queue: IEvidUnit[] = [root];
...
function replacePortableTokens(
  input: string,
  replacements: EvidPortableReplacement[],
): string {
  ...
  const ordered: EvidPortableReplacement[] = Array.from(unique.entries()).sort(
    (left: EvidPortableReplacement, right: EvidPortableReplacement): number => {
  ...
  while (cursor < input.length) {
    let selected: EvidPortableReplacement | undefined;
