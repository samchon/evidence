import type { IEvidGraphClaimContext } from "../contexts/IEvidGraphClaimContext";
import type { IEvidGraphReferenceContext } from "../contexts/IEvidGraphReferenceContext";
...
import type { IEvidGraphClaimResult } from "../structures/IEvidGraphClaimResult";
import type { IEvidGraphEdge } from "../structures/IEvidGraphEdge";
import type { IEvidGraphHostCoverage } from "../structures/IEvidGraphHostCoverage";
...
class EvidGraphEvaluator {
  ...
  private evaluateClaim(
    ...
    const inventory = new EvidInventory([claim.inventory]);
    const snapshot = inventory.snapshot();
    const population = inventory.select(claim.unitIds);
    const context: IEvidGraphClaimContext = {
      claim,
      inventory: snapshot,
      population,
      index: claimIndex,
  ...
  private evaluateReference(
    context: IEvidGraphClaimContext,
    ...
    const inventory = new EvidInventory([reference.inventory]);
    const snapshot = inventory.snapshot();
      ...
      for (const unit of selectedUnits) {
        ...
        this.diagnostics.push(
          this.problem(
            ...
            reference.severity,
            `Evid unit '${this.display(referenceInventory, unit)}' has ${hosts.size} distinct positive evidence host(s); uniqueEvid allows at most 1.`,
  ...
  private conflicts(
    ...
    unitIds: string[],
    edges: IEvidGraphEdge[],
    declarations: Map<string, IEvidDeclaration>,
  ...
  private evaluateReviews(
    ...
    const scopeIds = new Set(referencePopulation.scopes.map((unit) => unit.id));
    const acknowledgements: IEvidResolvedAcknowledgement[] = [];
    ...
    const records: IEvidResolvedReview[] = [];
   ...
   */
  private displayUnit(inventory: IEvidInventory, id: string): string {
  ...
  private resolutions(
    ...
    const records = new Map<string, IEvidGraphResolution>();
  ...
  private reviewResolutions(
    ...
    const records = new Map<string, IEvidGraphReviewResolution>();
    const conflicts = new Set<string>();
  ...
  private compareStatements(
    x: IEvidTargetStatement | undefined,
    y: IEvidTargetStatement | undefined,
    xId: string,
    yId: string,
  ): number {
  ...
  private descends(
    ...
    let current: IEvidUnit | undefined = unit;
  ...
  private display(inventory: IEvidInventory, unit: IEvidUnit): string {
    ...
    const address =
      addresses.find((candidate) =>
        selectedFiles.has(EvidFileTarget.normalize(candidate.file)),
      ) ?? addresses[0];
    if (address === undefined) return unit.name;
    return address.segments.length === 0
      ? address.file
      : EvidFileTarget.format(address);
  }
  ...
  private recordUnhostedChecklist(
    ...
    const previous: IEvidUnhostedChecklist | undefined =
