import type { IEvidDocumentationFence } from "./IEvidDocumentationFence";
...
export namespace EvidDocumentationExamples {
  ...
  ): IEvidDocumentationFence | undefined {
    let cursor: number = 0;
 ...
 */
function markupTags(input: string): IEvidHtmlMarkupTag[] {
  const output: IEvidHtmlMarkupTag[] = [];
  let cursor: number = 0;
