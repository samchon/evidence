...
import { EvidLanguageRegistry } from "./EvidLanguageRegistry";
import { EvidParseSession } from "./EvidParseSession";
import { EvidParserError } from "./EvidParserError";
import { EvidParserSlots } from "../internal/EvidParserSlots";
import { EvidTreeSitterAssets } from "../internal/EvidTreeSitterAssets";
import { EvidTreeSitterRange } from "../internal/EvidTreeSitterRange";
import { EvidTreeSitterRuntime } from "../internal/EvidTreeSitterRuntime";
import type { IEvidGrammar } from "../structures/IEvidGrammar";
import type { IEvidParserInput } from "../structures/IEvidParserInput";
import type { IEvidParserOptions } from "../structures/IEvidParserOptions";
...
export class EvidParser {
   ...
   */
  private readonly assets = new EvidTreeSitterAssets();
   ...
   */
  public async grammars(): Promise<IEvidGrammar[]> {
  ...
  public async parse<T>(
    ...
    closure: (session: EvidParseSession) => T | Promise<T>,
  ): Promise<T> {
    ...
    input = { ...input };
    const grammar = EvidLanguageRegistry.select(input.type, input.file);
    try {
    ...
    let session: EvidParseSession | undefined;
    try {
      ...
      if (tree.rootNode.hasError) {
        ...
        try {
          ...
          throw new EvidParserError(
            ...
            EvidTreeSitterRange.from(node),
          );
        } finally {
          cursor.delete();
        }
      }
      session = new EvidParseSession(tree, input.file);
  ...
  private async load(id: string): Promise<Language> {
    ...
    const language = await EvidTreeSitterRuntime.language(grammar, bytes);
    this.loaded.add(id);
    return language;
  }
}
