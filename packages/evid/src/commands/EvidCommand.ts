...
import { EvidChecker } from "../EvidChecker";
import { EvidCommandError } from "./EvidCommandError";
import { EvidGraphReporter } from "../reporters/EvidGraphReporter";
import { EvidQuery } from "../graph/EvidQuery";
import { EvidQueryReporter } from "../reporters/EvidQueryReporter";
import { EvidReporter } from "../reporters/EvidReporter";
import { EvidWatcher } from "./EvidWatcher";
import { EvidWatchReporter } from "../reporters/EvidWatchReporter";
...
import { EvidTreeSitterAssetScope } from "../internal/EvidTreeSitterAssetScope";
import type { IEvidPackageManifest } from "../internal/IEvidPackageManifest";
import type { IEvidCheckCommand } from "../structures/IEvidCheckCommand";
import type { IEvidCommand } from "../structures/IEvidCommand";
import type { IEvidCommandFailure } from "../structures/IEvidCommandFailure";
import type { IEvidCommandResult } from "../structures/IEvidCommandResult";
import type { IEvidGraphCommand } from "../structures/IEvidGraphCommand";
import type { IEvidInitCommand } from "../structures/IEvidInitCommand";
import type { IEvidInspectCommand } from "../structures/IEvidInspectCommand";
import type { IEvidLanguagesCommand } from "../structures/IEvidLanguagesCommand";
import type { IEvidListCommand } from "../structures/IEvidListCommand";
...
export namespace EvidCommand {
  ...
  export function parse(args: readonly string[]): IEvidCommand {
    ...
    for (let index = 0; index < tokens.length; index++) {
      ...
      if (!token.startsWith("-")) {
        ...
        if (target !== undefined)
          throw new EvidCommandError(
            "Evid inspect accepts exactly one target.",
          );
        target = token;
        continue;
      }
      ...
      if (value === undefined || value.startsWith("-"))
        throw new EvidCommandError(`Option '${token}' requires a value.`);
      if (value === "")
        throw new EvidCommandError(`Option '${token}' cannot be empty.`);
  ...
  ): Promise<IEvidCommandResult> {
    let parsed: IEvidCommand;
    try {
      parsed = parse(args);
    } catch (cause) {
  ...
  export async function main(
    args: readonly string[],
  ): Promise<EvidCommandExitCode> {
    let parsed: IEvidCommand | undefined;
    try {
      parsed = parse(args);
    } catch {
    ...
    const result = await EvidTreeSitterAssetScope.run(
      {
        progress: (message) => {
          if (
            selected !== undefined &&
...
async function runWatch(
  ...
  try {
    await watcher.watch(async (cycle) => {
      const content = EvidWatchReporter.render(cycle, command.format);
...
async function runAnalysis(
  ...
  const configFile = path.resolve(cwd, command.config);
  try {
    const analysis = await new EvidChecker(configFile).analyze();
    if (command.operation === "check")
      return writeReport(
        command.output,
        cwd,
        EvidReporter.render(analysis.report, command.format),
    ...
    const query = new EvidQuery(analysis, cwd);
    if (command.operation === "list") {
      const report = query.list(command.language, command.kind);
      return writeReport(
        command.output,
        cwd,
        EvidQueryReporter.render(report, command.format),
        report.exitCode,
        false,
      );
    }
    ...
    return writeReport(
      command.output,
      cwd,
      EvidGraphReporter.render(report, command.format),
      report.exitCode,
      false,
    );
  } catch (cause) {
        ...
        ? JSON.stringify(
            {
              ...
              message,
              repair,
            } satisfies IEvidCommandFailure,
            null,
            2,
          ) + "\n"
        : `Evid ${command.operation} failed: ${message}\nRepair: ${repair}\n`;
    return writeReport(
      command.output,
      cwd,
      output,
...
async function runLanguages(
  ...
  const cwd = path.resolve(baseCwd, command.cwd);
  const report = EvidQuery.languages();
  return writeReport(
    command.output,
    cwd,
    EvidQueryReporter.render(report, command.format),
...
async function runInit(
  ...
  const configFile = path.resolve(cwd, command.config);
  try {
    await EvidCommand.initialize(configFile);
...
): Exclude<IEvidCommand["operation"], "help" | "version"> {
 ...
 */
const INITIAL_DATA: IEvidConfig = {
  claims: [
    {
      name: "application",
      type: "typescript",
...
const INITIAL_CONFIG = dedent`
  ...
  export default ${JSON.stringify(INITIAL_DATA, null, 2)} satisfies IEvidConfig;
`;
