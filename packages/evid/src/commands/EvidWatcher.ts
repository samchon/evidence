...
import { EvidConfigDependencyScanner } from "../internal/EvidConfigDependencyScanner";
...
import type { IEvidConfigDependencyScan } from "../internal/IEvidConfigDependencyScan";
import type { IEvidWatchAttempt } from "../internal/IEvidWatchAttempt";
import { EvidSourcePath } from "../internal/EvidSourcePath";
import { EvidWatchDependencySet } from "../internal/EvidWatchDependencySet";
import { EvidWatchDependencySnapshot } from "../internal/EvidWatchDependencySnapshot";
...
import type { IEvidWatchOptions } from "../structures/IEvidWatchOptions";
import type { EvidWatchPublisher } from "../typings/EvidWatchPublisher";
...
export class EvidWatcher {
   ...
   */
  private active: IEvidSourceDependency[];
   ...
   */
  public dependencies(): IEvidSourceDependency[] {
  ...
  public async watch(publish: EvidWatchPublisher): Promise<void> {
    ...
    try {
      ...
      while (!this.isClosed()) {
        ...
        const changed = await EvidWatchDependencySnapshot.capture(this.active);
  ...
  private async evaluateStable(): Promise<IEvidWatchAttempt> {
    for (;;) {
      if (this.isClosed()) {
        const snapshot = await EvidWatchDependencySnapshot.capture(this.active);
        return {
          cycle: failureCycle(
            this.cycles + 1,
            this.configFile,
      ...
      const candidates = EvidWatchDependencySet.merge(
        this.active,
        beforeConfig.dependencies,
      );
      const before = await EvidWatchDependencySnapshot.capture(candidates);

      let analysis: IEvidCheckAnalysis | undefined;
      let analysisCause: unknown;
      try {
        analysis = await EvidChecker.analyze(this.configFile);
      } catch (cause) {
        analysisCause = cause;
      }
        ...
        analysis === undefined
          ...
          : cause === undefined
            ...
            : EvidWatchDependencySet.merge(
                this.active,
                afterConfig.dependencies,
                EvidWatchDependencySet.analysis(analysis, []),
              );
      ...
      if (!EvidWatchDependencySet.contains(candidates, active)) {
        this.active = active;
      ...
      const after = await EvidWatchDependencySnapshot.capture(
        EvidWatchDependencySet.merge(candidates, active),
      );
  ...
  private async settle(
    ...
    while (!this.isClosed() && this.debounceMilliseconds !== 0) {
      ...
      const current = await EvidWatchDependencySnapshot.capture(this.active);
...
async function scanConfiguration(
  configFile: string,
): Promise<IEvidConfigDependencyScan> {
  const scanner = new EvidConfigDependencyScanner(configFile);
  try {
...
function parserFailure(cause: unknown): boolean {
  return (
    cause instanceof EvidParserError &&
...
function configurationFallback(configFile: string): IEvidSourceDependency[] {
  ...
  return [
    ...
    { path: EvidSourcePath.slash(path.dirname(normalized)), recursive: true },
  ];
}
