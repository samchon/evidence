...
import type { IPrismaCommentRun } from "./internal/IPrismaCommentRun";
import type { IPrismaFileAnalysis } from "./internal/IPrismaFileAnalysis";
import type { IPrismaLocatedDeclaration } from "./internal/IPrismaLocatedDeclaration";
import type { IPrismaModel } from "./internal/IPrismaModel";
import type { IPrismaOwnedUnit } from "./internal/IPrismaOwnedUnit";
import type { IPrismaSchemaFile } from "./internal/IPrismaSchemaFile";
import { InventoryMerge } from "./internal/InventoryMerge";
import { PrismaFileScanner } from "./internal/PrismaFileScanner";
import { PrismaModelLoader } from "./internal/PrismaModelLoader";
...
export class EvidencePrismaAdapter implements IEvidenceAdapter {
  ...
  public async analyze(
    ...
    let models: IPrismaModel[];
    try {
      models = (await PrismaModelLoader.load(input.root.absolute, files))
        .models;
    } catch (cause) {
        ...
        return {
          name:
            source.addresses
              ...
              .sort(InventoryMerge.compare)[0] ?? source.physicalPath,
          source,
        };
      })
      .sort((x, y) => InventoryMerge.compare(x.name, y.name));
  }

  private units(
    inventory: IEvidenceInventory,
    models: IPrismaModel[],
    analyses: IPrismaFileAnalysis[],
    files: IPrismaSchemaFile[],
  ): Map<string, IPrismaOwnedUnit> {
    ...
    const output = new Map<string, IPrismaOwnedUnit>();
  ...
  private unit(
    ...
    digest: string,
    locations: Map<string, IPrismaLocatedDeclaration>,
  ...
  private publish(
    inventory: IEvidenceInventory,
    units: Map<string, IPrismaOwnedUnit>,
    owned: IPrismaOwnedUnit,
  ): void {
  ...
  private materialize(
    inventory: IEvidenceInventory,
    analyses: IPrismaFileAnalysis[],
    owned: Map<string, IPrismaOwnedUnit>,
  ): void {
    const units = new Map(
  ...
  private host(
    ...
    run: IPrismaCommentRun,
    entry: IPrismaOwnedUnit | undefined,
    attached: boolean,
  ): IEvidenceHost {
  ...
  private recoverWithdrawal(entry: IPrismaOwnedUnit): void {
  ...
  private reportBuried(
    ...
    const sourceText = new SourceText(source.content);
    let offset = 0;
