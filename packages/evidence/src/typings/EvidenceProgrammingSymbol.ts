 ...
 *   - Members initialized with functions or annotated with an inline function
 *     type. Overload declarations form one unit.
 * - Properties include:
 ...
 *     regardless of the constructor's visibility.
 *     Property modifiers include public, protected, private, readonly, and
 *     override; documentation belongs on the parameter.
 ...
 *   - Classes/namespaces themselves remain visible, but class members and
 *     namespace values do not.
 ...
 *   use Type.member; interfaces merged with classes use Class.prototype.member.
 * - File-qualified targets use a path relative to the citing file, "#", and the
 *   accessor. Literal member names use bracketed JSON strings. Symbol characters
 *   match exactly; path normalization must not rewrite them.
