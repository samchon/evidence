...
/**
 * Published watch cycle containing a fresh, stable Evidence check report.
 *
 ...
 * is still publishable; operational exceptions use the separate failure envelope.
 */
export interface IEvidenceWatchCheckCycle {
  /**
   * Version of the watch envelope's serialization contract.
   *
   * Readers can validate it before interpreting the nested report and cycle fields.
   */
  schemaVersion: 1;
   ...
   *
   * Consumers use cycle numbers to distinguish repeated evaluations of one command.
   */
  watch: true;
   ...
   *
   * Stable failures and successful analyses share the same publication sequence;
   * discarded unstable attempts do not consume another number.
   */
  cycle: number;
   ...
   *
   * Complete describes analysis availability, not whether coverage errors were found.
   */
   ...
   *
   * Zero passes, one reports completed errors, and two reports incomplete analysis;
   * publication itself does not end the watcher.
   */
  exitCode: 0 | 1 | 2;
   ...
   *
   * It retains independent claims, counts, and actionable diagnostics for this cycle.
   */
