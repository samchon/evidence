 ...
 * resolution preserve every public spelling without duplicating coverage units.
 *
 * An address exposes a declaration but does not establish its structural owner.
 * Parent relationships remain on the unit, even when an alias changes the apparent
 * nesting of the public path.
 */
export interface IEvidencePublicAddress extends IEvidenceAddress {
  /**
   * Semantic identity exposed through the inherited address.
   *
   * The ID must exist in the inventory. Multiple addresses may share it; an exact
   * address associated with different IDs remains an ambiguous target.
   */
  unitId: string;
}
