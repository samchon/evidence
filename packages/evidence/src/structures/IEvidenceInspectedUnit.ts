...
/**
 * Candidate identity expanded with its immediate structural and documentation context.
 *
...
export interface IEvidenceInspectedUnit {
  /**
   * Candidate's public addresses, declaration sites, and population selection state.
   *
   * Its row ID retains the configuration boundary in which the candidate was inspected.
   */
   ...
   *
   * This is immediate context, not a recursive flattening or a name-prefix search.
   */
   ...
   *
   * Hosts are ordered by ID and can also belong to other units in a shared statement.
   */
  hosts: IEvidenceHost[];

  /**
   * Current own-content and subtree fingerprint context when the inventory is complete.
   *
   * Omission means extraction cannot certify a fingerprint; it is not an empty hash.
   */
