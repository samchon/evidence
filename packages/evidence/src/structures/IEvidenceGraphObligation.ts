 ...
 * outcome. Accepted edges explain how statements contributed coverage. Checklist
 * mode additionally records each host's answers instead of pooling them across
 * the claim.
 *
...
export interface IEvidenceGraphObligation {
   ...
   *
   * Repeated reference entries keep separate indices, policies, and coverage even
   * when they require the same semantic target population.
   */
  reference: number;
   ...
   * denominator. Missing evidence in a fully analyzed population is a separate state.
   */
  complete: boolean;
   ...
   *
   * Public aliases and unselected structural ancestors do not add entries. Other
   * references select and evaluate their own ID lists independently.
   */
  unitIds: string[];
   ...
   *
   * Coverage belongs to this boundary. The same identity may remain missing in a
   * stricter reference or under a different claim.
   */
   ...
   *
   * These are uncovered requirements, not unresolved source declarations. Inspect
   * completeness before interpreting a partial evaluation's coverage lists.
   */
   ...
   *
   * Each edge retains its source host, named target, and covered selected units.
   * Review records do not appear here as substitute coverage.
   */
   ...
   *
   * Ordinary obligations leave the list empty. Checklist entries preserve missing
   * answers for each host even when another host covers the same target item.
   */
