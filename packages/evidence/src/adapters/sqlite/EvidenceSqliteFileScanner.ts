...
import { EvidenceSqliteSyntax } from "./EvidenceSqliteSyntax";
 ...
 */
export class EvidenceSqliteFileScanner {
  /**
   ...
   */
  private readonly declarations: IEvidenceSqlDeclaration[] = [];
  ...
  private table(node: EvidenceNode): void {
    ...
    const names = EvidenceSqliteSyntax.names(node);
    ...
    if (temporary && EvidenceSqliteSyntax.fold(schema) !== "temp") {
      this.incomplete(node, "A TEMP table may only name the temp schema.");
      return;
    }
    if (EvidenceSqliteSyntax.fold(name).startsWith("sqlite_")) {
      this.incomplete(
        node,
    ...
    const identity = [schema, name].map(EvidenceSqliteSyntax.fold);
    ...
    const columnNames = columns.flatMap((column) => {
      const columnName = EvidenceSqliteSyntax.names(column)[0];
      return columnName === undefined
        ? []
        : [EvidenceSqliteSyntax.fold(columnName)];
    });
    for (const column of columns) {
      ...
      this.declare(
        ...
        "column",
        [...names, columnName],
        [...identity, EvidenceSqliteSyntax.fold(columnName)],
        model.id,
      );
  ...
  private relation(
    ...
    const remote = clause.namedChildren
      ...
      .flatMap((child) => {
        const name = EvidenceSqliteSyntax.identifier(child);
    ...
    this.declare(
      ...
      [...model.identity, EvidenceSqliteSyntax.fold(segment)],
      model.id,
    );
  }
   ...
   */
  private documentation(): IEvidenceSqlDocumentation[] {
    const comments = this.session
      .captures("(comment) @comment")
      .map((capture) => capture.node)
      .sort((left, right) => left.startIndex - right.startIndex);
    const groups: EvidenceNode[][] = [];
   ...
   */
  private leading(node: EvidenceNode): boolean {
    return (
      this.source.content
        .slice(
   ...
   */
  private incomplete(node: EvidenceNode, message: string): void {
