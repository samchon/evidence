...
export class ZigAdapter implements IEvidenceAdapter {
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, IZigDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidenceTagParser.parse(
      source.content,
      host,
      ZigDocumentation.read(source, documentation, host.id),
    );
  }
  ...
  private annotation(
    ...
    return this.annotationPattern(
      ZigDocumentation.read(analysis.source, documentation, documentation.id)
        .text,
      true,
    );
  }
