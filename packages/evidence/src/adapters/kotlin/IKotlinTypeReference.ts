 ...
 *
 * KotlinFileScanner records lookup candidates while file scope and imports are
 * available, and KotlinReceivers resolves the reference after declarations and
...
export interface IKotlinTypeReference {
   ...
   *
   * KotlinReceivers permits file-private aliases only from this file, preserving
   ...
   *
   * KotlinReceivers checks these paths in order and accepts exactly one visible
   * declaration, so their ordering represents KotlinFileScanner's precedence.
   */
  paths: string[][];
   ...
   *
   * Omission permits KotlinReceivers lookup; when present, callers preserve the
