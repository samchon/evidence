...
import { DartDocumentation } from "./DartDocumentation";
...
export class DartAdapter implements IEvidenceAdapter {
  ...
  public async analyze(
    ...
    try {
      ...
      DartLibraries.publish(analyses, inventory, published);
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, IDartDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidenceTagParser.parse(
      source.content,
      host,
      DartDocumentation.read(source, documentation, host.id),
    );
  }
  ...
  private annotation(
    ...
    return this.annotationPattern(
      DartDocumentation.read(analysis.source, documentation, documentation.id)
        .text,
      true,
    );
  }
