...
export class LuaAdapter implements IEvidenceAdapter {
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, ILuaDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidenceTagParser.parse(
      source.content,
      host,
      LuaDocumentation.read(source, documentation, host.id),
    );
  }
  ...
  private annotation(
    ...
    return this.annotationPattern(
      LuaDocumentation.read(analysis.source, documentation, documentation.id)
        .text,
      true,
    );
  }
