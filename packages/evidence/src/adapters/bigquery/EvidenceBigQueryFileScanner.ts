...
import { EvidenceBigQueryColumnType } from "./EvidenceBigQueryColumnType";
import { EvidenceBigQueryIdentifier } from "./EvidenceBigQueryIdentifier";
import { EvidenceBigQueryString } from "./EvidenceBigQueryString";
import { EvidenceBigQueryWithdrawals } from "./EvidenceBigQueryWithdrawals";
 ...
 */
export class EvidenceBigQueryFileScanner {
  /**
   ...
   * belongs here.
   */
  private readonly output: IEvidenceSqlFileAnalysis;
   ...
   * returned.
   */
  public scan(): IEvidenceSqlFileAnalysis {
    ...
    EvidenceBigQueryWithdrawals.apply(this.output);
    return this.output;
  }
  ...
  private table(node: EvidenceNode): void {
    ...
    const path = EvidenceBigQueryIdentifier.table(name.text);
    if (path === undefined) {
      this.fail(
        name,
  ...
  private column(
    ...
    const name =
      identifier === null
        ? undefined
        : EvidenceBigQueryIdentifier.column(identifier.text);
    ...
    if (!EvidenceBigQueryColumnType.supported(type))
      this.fail(
        type,
  ...
  private constraint(
    ...
    const names =
      list === null
        ...
        : list.namedChildren.map((child) =>
            EvidenceBigQueryIdentifier.canonicalColumn(child.text),
          );
    const available = columns
      ...
      .map((name) =>
        name === null
          ? undefined
          : EvidenceBigQueryIdentifier.canonicalColumn(name.text),
      );
    if (
      names.length === 0 ||
    ...
    const constraintName =
      identifier === null
        ? undefined
        : EvidenceBigQueryIdentifier.member(identifier.text);
   ...
   * operational meaning.
   */
  private enforcement(node: EvidenceNode): void {
  ...
  private options(node: EvidenceNode, declaration: IEvidenceSqlDeclaration): void {
    ...
    const descriptions = clause.namedChildren.filter(
      (child) =>
        ...
        EvidenceBigQueryIdentifier.canonical(
    ...
    for (const item of descriptions) {
      ...
      const mapped =
        value?.type === "string"
          ? EvidenceBigQueryString.read(value.text, value.startIndex)
          : undefined;
   ...
   */
  private comment(node: EvidenceNode, last: EvidenceNode): void {
  ...
  private declare(
    ...
    owner: IEvidenceSqlDeclaration | undefined,
    visible: boolean,
   ...
   * pass coverage.
   */
  private fail(node: EvidenceNode, message: string): void {
