...
import { EvidenceMysqlIdentifier } from "./EvidenceMysqlIdentifier";
 ...
 * server state.
 */
export namespace EvidenceMysqlPolicy {
  /**
   ...
   * setting.
   */
  export const identifier = EvidenceMysqlIdentifier.read;
 ...
 * snapshot.
 */
function tableOption(node: EvidenceNode): string | undefined {
  const raw = node.text;
  if (
