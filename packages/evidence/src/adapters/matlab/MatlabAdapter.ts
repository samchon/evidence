...
import { MatlabDocumentation } from "./MatlabDocumentation";
...
export class MatlabAdapter implements IEvidenceAdapter {
  ...
  private materializeUndocumentedHosts(
    ...
    const groups = new Map<string, IMatlabDeclaration[]>();
    for (const declaration of analysis.declarations) {
  ...
  private parse(
    ...
    return EvidenceTagParser.parse(
      source.content,
      host,
      MatlabDocumentation.read(source, documentation, host.id),
    );
  }
  ...
  private annotation(
    ...
    return this.annotationPattern(
      MatlabDocumentation.read(analysis.source, documentation, documentation.id)
        .text,
      true,
    );
  }
