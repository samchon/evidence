...
import type { IPrismaCacheEntry } from "./IPrismaCacheEntry";
import type { IPrismaDatamodel } from "./IPrismaDatamodel";
import type { IPrismaDatamodelModel } from "./IPrismaDatamodelModel";
import type { IPrismaLoadResult } from "./IPrismaLoadResult";
import type { IPrismaModel } from "./IPrismaModel";
import type { IPrismaParser } from "./IPrismaParser";
...
export namespace PrismaModelLoader {
  export async function load(
    ...
    let datamodel: IPrismaDatamodel;
    try {
      datamodel = typia.json.assertParse<IPrismaDatamodel>(payload);
    } catch (cause) {
      throw new Error(
  ...
  function model(input: IPrismaDatamodelModel): IPrismaModel {
    return {
      name: input.name,
      documentation: input.documentation ?? "",
      digest: CanonicalJson.digest(
        CanonicalJson.without(input, ["documentation", "fields"]),
      ),
      fields: input.fields.map((field) => ({
        ...
        digest: CanonicalJson.digest(
          CanonicalJson.without(field, ["documentation"]),
        ),
      })),
    };
  }

  function resolve(root: string): IPrismaParser {
  ...
  function schemaDigest(files: IPrismaSchemaFile[]): string {
  ...
  function remember(key: string, entry: IPrismaCacheEntry): void {
