...
export interface IEcmaScriptCommentAttachment {
   ...
   *
   * It remains separate from the unit so merged declarations retain attachment truth. */
  siteId: string;

  /** Semantic unit affected by this JSDoc carrier.
   *
   * Several units can share one physical host only when the scanner establishes it. */
  unitId: string;

  /** Limits the carrier to visibility withdrawal processing.
   *
   * Constructor documentation can hide parameter properties but cannot claim evidence. */
