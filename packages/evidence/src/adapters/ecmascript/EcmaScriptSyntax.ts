...
export namespace EcmaScriptSyntax {
  ...
  export function qualifiedName(node: Node | null): string[] {
    if (node === null) return [];
  ...
  export function modifier(node: Node, value: string): boolean {
    return node.children.some(
      (child) =>
        child.type === value ||
  ...
  export function functionType(node: Node | null): boolean {
    let current = node;
  ...
  export function jsdoc(node: Node): boolean {
    return node.type === "comment" && node.text.startsWith("/**");
  }

  export function module(source: Node | null): string | undefined {
    if (source === null || source.type !== "string") return undefined;
    return string(source.text);
  }

  export function bindings(pattern: Node | null): Node[] {
