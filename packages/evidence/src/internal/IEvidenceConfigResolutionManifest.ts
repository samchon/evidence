 ...
 *
 * EvidenceConfigDependencyScanner validates this narrow projection from fresh
 * manifest bytes on every watch cycle. Other package fields cannot select a
 * runtime entry or package scope and therefore remain outside the scanner's
 * resolution contract.
 */
export interface IEvidenceConfigResolutionManifest {
  /**
   ...
   * Node's `index` fallback; subpath requests do not use this root field
   * because they already name their legacy path.
   */
  main?: unknown;
   ...
   * unexported or condition-incompatible requests explicit resolution
   * failures.
   */
  exports?: unknown;
