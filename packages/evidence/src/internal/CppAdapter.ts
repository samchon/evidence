...
import { CppDocumentation } from "./CppDocumentation";
import { CppFileScanner } from "./CppFileScanner";
import type { ICppAlias } from "./ICppAlias";
import type { ICppDeclaration } from "./ICppDeclaration";
import type { ICppDeclarationGroup } from "./ICppDeclarationGroup";
...
export class CppAdapter implements IEvidenceAdapter {
  ...
  private materializeUnits(
    ...
    analyses: ICppFileAnalysis[],
  ): Map<string, string> {
    const declarationAnalysis = new Map<string, ICppFileAnalysis>();
    const groups = new Map<string, ICppDeclarationGroup>();
    for (const analysis of analyses)
  ...
  private checkGroup(
    inventory: IEvidenceInventory,
    group: ICppDeclarationGroup,
    analyses: Map<string, ICppFileAnalysis>,
  ): void {
  ...
  private resolveAliases(
    ...
    const output: ICppResolvedAlias[] = [];
  ...
  private parse(
    ...
    return EvidenceTagParser.parse(
      source.content,
      host,
      CppDocumentation.read(source, documentation, host.id),
    );
  }
  ...
  private annotation(
    ...
    return this.annotationPattern(
      CppDocumentation.read(analysis.source, documentation, documentation.id)
        .text,
      true,
    );
  }
  ...
  private unitId(declaration: ICppDeclaration): string {
