...
import { ConfigPackageMainError } from "./ConfigPackageMainError";
import { ConfigPackageTargetError } from "./ConfigPackageTargetError";
import type { EvidenceConfigModuleMode } from "./EvidenceConfigModuleMode";
import type { IConfigDataModule } from "./IConfigDataModule";
import type { IConfigModuleSpecifier } from "./IConfigModuleSpecifier";
import type { IConfigPackageBoundary } from "./IConfigPackageBoundary";
...
export class ConfigDependencyScanner {
  ...
  public async scan(): Promise<IEvidenceSourceDependency[]> {
    try {
      const logical: string = SourcePath.slash(path.resolve(this.configFile));
      this.watch(logical, false);
      this.watch(SourcePath.slash(path.dirname(logical)), true);
      const physical: string = SourcePath.slash(await realpath(logical));
  ...
  private async scanFile(file: string): Promise<void> {
    ...
    const specifiers: IConfigModuleSpecifier[] = await this.parser.parse(
      { type, file: physical, content },
      (session: EvidenceParseSession): IConfigModuleSpecifier[] =>
  ...
  private async resolve(
    owner: string,
    request: IConfigModuleSpecifier,
  ): Promise<string[]> {
  ...
  private async resolveData(specifier: string): Promise<string[]> {
    ...
    const data: IConfigDataModule = configDataModule(specifier);
    if (data.content === undefined) return [];
    const requests: IConfigModuleSpecifier[] = await this.parser.parse(
  ...
  private async resolvePackageImport(
    ...
    const scope: IConfigPackageBoundary | undefined =
  ...
  private async resolveCommonJsPackage(
    ...
    for (;;) {
      const nodeModules: string = SourcePath.slash(
        path.join(directory, "node_modules"),
      );
      const packageDirectory: string = SourcePath.slash(
        path.join(nodeModules, packageName),
      );
      this.watch(nodeModules, true);
      this.watch(SourcePath.slash(path.dirname(packageDirectory)), true);
  ...
  private async packageBoundary(
    owner: string,
  ): Promise<IConfigPackageBoundary | undefined> {
    let directory: string = path.dirname(owner);
    for (;;) {
      const manifestFile: string = SourcePath.slash(
        path.join(directory, "package.json"),
      );
      this.watch(manifestFile, false);
      try {
        if ((await stat(manifestFile)).isFile()) {
          const manifest: IConfigResolutionManifest = parseResolutionManifest(
            await readFile(manifestFile, "utf8"),
          );
          return { directory: SourcePath.slash(directory), manifest };
        }
      } catch (cause) {
        if (!absent(cause)) throw cause;
      }
  ...
  private async moduleMode(file: string): Promise<EvidenceConfigModuleMode> {
    ...
    for (;;) {
      const manifest: string = SourcePath.slash(
        path.join(directory, "package.json"),
      );
      this.watch(manifest, false);
      try {
        if ((await stat(manifest)).isFile()) {
          const scope: IConfigPackageScope = parsePackageScope(
   ...
   *
   * Explicit MTS/MJS and CTS/CJS extensions fix their Node semantics. JavaScript
  ...
  /**
   * Resolves one package request without reusing process-global Node path caches.
   *
  ...
  private async resolvePackageDirectory(
    ...
    this.watch(manifestFile, false);
    let manifest: IConfigResolutionManifest = {};
    try {
  ...
  private async resolveExact(file: string): Promise<string[]> {
    const location: string = SourcePath.slash(path.resolve(file));
  ...
  private async resolveRuntimePath(
    ...
    const location: string = SourcePath.slash(path.resolve(base));
      ...
      if ((await stat(location)).isDirectory()) {
        ...
        try {
          ...
          if (mainTarget !== undefined && resolutionMissing(cause))
            throw new ConfigPackageMainError(location, mainTarget, mainFailure);
          throw cause;
        }
      }
    } catch (cause) {
  ...
  private async resolveRuntimeIndex(directory: string): Promise<string[]> {
    const base: string = SourcePath.slash(path.join(directory, "index"));
    ...
    this.watch(SourcePath.slash(directory), true);
 ...
 */
function parseResolutionManifest(input: string): IConfigResolutionManifest {
  return typia.json.assertParse<IConfigResolutionManifest>(packageJson(input));
}
 ...
 */
function parsePackageScope(input: string): IConfigPackageScope {
  return typia.json.assertParse<IConfigPackageScope>(packageJson(input));
}
 ...
 */
function configDataModule(specifier: string): IConfigDataModule {
...
function collectSpecifiers(
...
): IConfigModuleSpecifier[] {
  const output: Map<string, IConfigModuleSpecifier> = new Map<
    string,
    IConfigModuleSpecifier
  >();
  ...
  for (const statement of session.root.namedChildren) {
    ...
    const direct: string | undefined = EcmaScriptSyntax.module(
    ...
    if (direct !== undefined) add(direct, staticMode);
      ...
      const literal: Node | undefined = statement
        ...
        .at(-1);
      const nested: string | undefined = EcmaScriptSyntax.module(
        literal ?? null,
      );
  ...
  for (const call of session.root.descendantsOfType("call_expression")) {
    ...
    const specifier: string | undefined = EcmaScriptSyntax.module(
      argument ?? null,
    );
...
function importBindsRequire(clause: Node): boolean {
  for (const child of clause.namedChildren) {
    ...
    for (const specifier of child.descendantsOfType("import_specifier")) {
      if (EcmaScriptSyntax.token(specifier, "type")) continue;
...
function strictDirective(scope: Node): boolean {
  for (const statement of scope.namedChildren) {
    ...
    if (EcmaScriptSyntax.module(expression) === "use strict") return true;
  }
  return false;
}
...
/**
 * Converts a legacy ESM package subpath into the physical pathname Node executes.
 *
...
function conditionalTarget(
  ...
  if (Array.isArray(value)) {
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (!(cause instanceof ConfigPackageTargetError)) throw cause;
        failure = cause;
        blocked = false;
      }
    }
  ...
  if (!record(value))
    throw new ConfigPackageTargetError(
 ...
 *
 * Node checks the authored path before its JavaScript, JSON, and native suffixes.
...
function runtimeCandidates(base: string): string[] {
  return unique([base, `${base}.js`, `${base}.json`, `${base}.node`]).map(
    (candidate: string): string => SourcePath.slash(candidate),
  );
}
