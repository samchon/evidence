...
export class RubyAdapter implements IEvidenceAdapter {
  ...
  private materializeUnits(
    ...
    const declarationAnalysis = new Map<string, IRubyFileAnalysis>();
    const groups = new Map<string, IRubyDeclaration[]>();
    for (const analysis of analyses)
  ...
  private checkGroup(
    ...
    declarationAnalysis: Map<string, IRubyFileAnalysis>,
  ): void {
    const first = declarations[0];
  ...
  private checkAddressCollisions(
    ...
    const firstDeclarations = new Map<string, IRubyDeclaration>();
    const declarationAnalysis = new Map<string, IRubyFileAnalysis>();
    for (const analysis of analyses)
      for (const declaration of analysis.declarations) {
        const unitId = published.get(declaration.id);
  ...
  private materializeUndocumentedHosts(
    ...
    for (const analysis of analyses) {
      ...
      const sites = new Map<string, IRubyDeclaration[]>();
      for (const declaration of analysis.declarations) {
        const unitId = published.get(declaration.id);
        if (
          unitId === undefined ||
  ...
  private unitId(declaration: IRubyDeclaration): string {
  ...
  private problem(
    inventory: IEvidenceInventory,
    analysis: IRubyFileAnalysis | undefined,
    declaration: IRubyDeclaration,
    code: string,
    message: string,
    repair: string,
  ): void {
