...
import type { IRustPublicOccurrence } from "./IRustPublicOccurrence";
...
export class RustModuleResolver {
  ...
  private readonly exports = new Map<
    string,
    Map<string, IRustExportRecord[]>
  >();
  private readonly externalTargets = new Map<string, IRustFileAnalysis>();
  private readonly identities = new Map<string, string>();
  private readonly implMembers = new Map<string, IRustResolvedMember[]>();
  private readonly modules = new Map<string, IRustModuleRecord>();
  private readonly placements = new Map<string, IRustFilePlacement>();
  ...
  private resolveExternalTargets(): void {
    const sources = new Map<string, IRustFileAnalysis[]>();
          ...
          new Map(
            this.sourceLocations(analysis)
              .flatMap((location) => {
                ...
                return [
                  SourcePath.slash(path.join(directory, `${external.name}.rs`)),
                  SourcePath.slash(
  ...
  private placeFiles(): void {
    ...
    const queue: IRustFilePlacement[] = [];
    for (const analysis of this.analyses)
  ...
  private attachExternalInnerDocumentation(): void {
    const sourceDeclarations = new Map<string, IRustDeclaration>();
    for (const analysis of this.analyses)
  ...
  private buildModules(): void {
    ...
    const rawDeclarations = new Map<string, IRustDeclaration>();
    for (const analysis of this.analyses)
      for (const declaration of analysis.declarations)
    ...
    for (const placement of this.placements.values())
      for (const declaration of placement.analysis.declarations) {
        ...
        if (
          ...
          const located: IRustLocatedDeclaration = {
            declaration,
            placement,
            module,
  ...
  private buildExports(): void {
    for (const module of this.modules.values()) {
      const entries = new Map<string, IRustExportRecord[]>();
    ...
    for (const placement of this.placements.values())
      for (const implementation of placement.analysis.implementations) {
        ...
        for (const member of members) {
          ...
          const resolved: IRustResolvedMember = {
            declaration: located,
            owner,
            ...(qualifier === undefined ? {} : { qualifier }),
            ...(trait[0] === undefined
  ...
  private collectOccurrences(): IRustPublicOccurrence[] {
    const output: IRustPublicOccurrence[] = [];
  ...
  private materialize(occurrences: IRustPublicOccurrence[]): void {
  ...
  private materializeOwnedMembers(
    occurrence: IRustPublicOccurrence,
    ownerUnit: IEvidenceUnit,
    reachable: Set<string>,
    roots: Map<string, IRustFilePlacement>,
  ): void {
  ...
  private materializeMember(
    occurrence: IRustPublicOccurrence,
    ownerUnit: IEvidenceUnit,
    member: IRustLocatedDeclaration,
    suffix: string[],
    root: IRustFilePlacement | undefined,
  ): void {
  ...
  private unitId(located: IRustLocatedDeclaration): string {
  ...
  private addExport(
    entries: Map<string, IRustExportRecord[]>,
    record: IRustExportRecord,
  ): boolean {
  ...
  private uniqueExports(records: IRustExportRecord[]): IRustExportRecord[] {
    return Array.from(
      new Map(
  ...
  private uniqueLocated(
    records: IRustLocatedDeclaration[],
  ): IRustLocatedDeclaration[] {
    return Array.from(
      new Map(
        records.map((record) => [record.declaration.id, record]),
  ...
  private sourceLocations(analysis: IRustFileAnalysis): string[] {
    return Array.from(
      new Set([
        SourcePath.slash(analysis.source.physicalPath),
        ...analysis.source.addresses.map((address) =>
          SourcePath.slash(address.absolute),
        ),
      ]),
    );
  }
  ...
  private implProblem(
    implementation: IRustImplementation,
  ...
  private problem(
    code: string,
    analysis: IRustFileAnalysis | undefined,
    message: string,
    repair: string,
    key: string,
  ): void {
