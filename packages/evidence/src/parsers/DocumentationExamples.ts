import type { IDocumentationFence } from "./IDocumentationFence";
 ...
 * adapter looks for Evidence tags; otherwise a literal `<pre>` in one fence can
 * consume real prose and annotations after that fence.
 */
export namespace DocumentationExamples {
  /**
  ...
  /**
   * Masks HTML comments before Evidence tag lines are classified.
   *
 ...
 */
function markupTags(input: string): IHtmlMarkupTag[] {
  const output: IHtmlMarkupTag[] = [];
  let cursor: number = 0;
