...
export namespace DbmlCertificationFixture {
  ...
  export function create(): IDatabaseAdapterCertification {
    ...
    return {
      type: "dbml", adapter: new EvidenceDbmlAdapter(),
      sources: [{ file, content: dedent`
      ...
      ` }],
      units: [unit(file, "model", ["public", "users"]), unit(file, "column", ["public", "users", "id"]), unit(file, "model", ["public", "posts"]), unit(file, "column", ["public", "posts", "user_id"]), unit(file, "relation", ["public", "posts", "$ref:owner"]), { ...unit(file, "model", ["public", "hidden"]), withdrawals: ["internal"] }, unit(file, "column", ["public", "hidden", "id"])],
      hosts: [model, key("column", ["public", "users", "id"]), key("model", ["public", "posts"]), column, relation].map((name) => ({ attachment: "attached", units: [name] })),
      requirements: [{ unit: model, target: "../docs/requirements.md#model" }, { unit: column, target: "../docs/requirements.md#column" }, { unit: relation, target: "../docs/requirements.md#relation" }],
      excludedUnits: ["model:status"], annotationRanges: 4,
      incomplete: { sources: [{ file, content: "TablePartial common { id int }" }], diagnosticCodes: ["dbml-unsupported-syntax"] },
      malformed: { sources: [{ file, content: "Table users { id int" }], diagnosticCodes: ["dbml-parse-incomplete"] },
      falsePositive: { source: { file, content: dedent`
      ...
      ` }, attachedTarget: "../docs/requirements.md#attached", unsupportedAnnotations: 0 },
      mutation: { unit: model, reasonBefore: "Persists the account identity.", reasonAfter: "Records the stable account identity.", contentBefore: "id int [pk]", contentAfter: "id bigint [pk]" },
    };
  }
  ...
  function unit(file: string, symbol: EvidenceDatabaseSymbol, identity: string[]): IDatabaseAdapterCertificationUnit {
    return { key: key(symbol, identity), symbol, identity, ...(symbol === "model" ? {} : { parent: key("model", identity.slice(0, 2)) }), sites: 1, addresses: [identity, identity.slice(1)].map((segments) => ({ file, accessor: EvidenceAccessor.format(segments) })), withdrawals: [] };
  }
}
