...
The root `README.md` is the only editable package README. `scripts/prepare-package.mts` copies it and LICENSE to the package during `prepack`. Keep repository links usable from npm as well as GitHub, and validate this copy through `pnpm verify:package` after packaging changes.
