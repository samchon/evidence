...
- **A merged identity resolves from the declaration encountered first.** Declaration merging and overload sets spell one unit through several declarations, and the unit reports the earliest one, so every diagnostic naming its line names that declaration. Order is source position, not declaration kind: `namespace ISale` written above `interface ISale` is the one reported. A citation may sit on any of them and resolves the same way, because the relation is judged on the identity rather than on the declaration carrying the tag. The graph's unit model decides what merges — a class is not a type unit, so `class Sale` beside `namespace Sale` is one declaration of `Sale` here even though `evidence/singular` and `evidence/documented` treat the pair as one identity.
