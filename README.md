...
| Prisma / MySQL | Yes | Yes | `model`, `column`, `relation` | All / `model` |
...
Database schema typings share `IEvidenceDatabaseClaim` and `IEvidenceDatabaseReference`. Prisma and MySQL have independently configured database adapters. Select the schema language rather than the database server: MongoDB models written in Prisma use `type: "prisma"`.
