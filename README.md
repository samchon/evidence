...
`EvidenceLanguageRegistry.list()` reports fifteen certified programming languages: TypeScript, JavaScript, Python, Go, Rust, Java, C#, C, C++, Ruby, Kotlin, Scala, Swift, MATLAB, and Objective-C. Syntax variants remain explicit: `.h` follows the configured `c`, `cpp`, or `objc` type, `.tsx` selects the TSX grammar within TypeScript, and JSX shares the JavaScript grammar. `select(type, file)` uses the configured language and case-sensitive logical file name. `parser.grammars()` reports pinned asset provenance without loading WASM; `parser.state()` exposes the instance's loaded grammar IDs and active/queued session counts. Grammar metadata alone does not establish public export resolution or graph coverage.
...
Database schema typings share `IEvidenceDatabaseClaim` and `IEvidenceDatabaseReference`. Prisma and portable SQL implement the database family. Select the schema language rather than the database server: MongoDB models written in Prisma use `type: "prisma"`.
